...
from resources.lib import torbox_advanced
from resources.lib import premiumize_hub
...
def cloud_service_menu(service):
    ...
    if service == 'rd':
        ...
        items.append(_menu_item('[B][COLOR yellowgreen]>> Premiumize Hub (Full features)[/COLOR][/B]', 'pm_hub'))
        items.append(_menu_item('[COLOR cyan]My Cloud Files[/COLOR]', 'pm_cloud_folder'))
        items.append(_menu_item('[COLOR cyan]Transfers[/COLOR]', 'pm_transfers'))
        items.append(_menu_item('[COLOR lime]+ Add Transfer[/COLOR]', 'pm_add_transfer_menu'))
...
def cloud_main_menu():
    ...
    # Rich TorBox hub (when TorBox is linked)
    if torbox_advanced.TorboxAdvanced().is_authorized():
        items.append(_menu_item(
            '[B][COLOR magenta]>> TorBox Hub  (Torrents · Usenet · Web DL · Search)[/COLOR][/B]',
            'tb_hub'))

    # Rich Premiumize hub (when PM is linked)
    if debrid.Premiumize().is_authorized():
        items.append(_menu_item(
            '[B][COLOR yellowgreen]>> Premiumize Hub  (Cloud · Transfers · Add · ZIP · Hosters)[/COLOR][/B]',
            'pm_hub'))
...
def cloud_service_browser(service):
    ...
    # For Premiumize, route to the rich hub directly
    if service == 'pm' and debrid.Premiumize().is_authorized():
        premiumize_hub.pm_hub()
        return
...
# ══════════════════════════════════════════════════════════════════════════
# TORBOX HUB - Rich TorBox-specific UI (torrents / usenet / web dl / search)
# ══════════════════════════════════════════════════════════════════════════

def tb_hub():
    """TorBox feature hub. Lists every operation Genesis exposes."""
    tb = torbox_advanced.TorboxAdvanced()
    if not tb.is_authorized():
        xbmcgui.Dialog().notification(
            'TorBox', 'Link TorBox first (Settings > Debrid)',
            xbmcgui.NOTIFICATION_WARNING)
        xbmcplugin.endOfDirectory(HANDLE)
        return

    items = [
        _menu_item('[B][COLOR cyan]Account Dashboard[/COLOR][/B]', 'tb_dashboard', is_folder=False),
        _menu_item('[B][COLOR lime]Torrents - All[/COLOR][/B]', 'tb_torrents'),
        _menu_item('[B][COLOR yellow]Torrents - Downloading[/COLOR][/B]', 'tb_torrents', extra_params={'filter': 'active'}),
        _menu_item('[B][COLOR lime]Torrents - Completed[/COLOR][/B]', 'tb_torrents', extra_params={'filter': 'done'}),
        _menu_item('[B][COLOR magenta]Search TorBox[/COLOR][/B]', 'tb_search', is_folder=False),
        _menu_item('[B][COLOR orange]Add Magnet/URL...[/COLOR][/B]', 'tb_add_magnet', is_folder=False),
        _menu_item('[B][COLOR skyblue]Usenet Downloads[/COLOR][/B]', 'tb_usenet_list'),
        _menu_item('[B][COLOR skyblue]Add NZB URL...[/COLOR][/B]', 'tb_add_nzb', is_folder=False),
        _menu_item('[B][COLOR gold]Web Downloads (DDL)[/COLOR][/B]', 'tb_webdl_list'),
        _menu_item('[B][COLOR gold]Add Web Download URL...[/COLOR][/B]', 'tb_add_webdl', is_folder=False),
    ]
    for url, li, is_folder in items:
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=is_folder)
    xbmcplugin.endOfDirectory(HANDLE)
...
def tb_dashboard():
    ...
    d = tb.dashboard()
    if not d:
        xbmcgui.Dialog().ok('TorBox', 'Could not fetch account info.\nKey may be invalid.')
        return
    body = (
        f"[B]Email:[/B] {d['email']}\n"
        f"[B]Plan:[/B] [COLOR yellow]{d['plan']}[/COLOR]\n"
        f"[B]Premium:[/B] {'Yes' if d['is_premium'] else 'No'}\n"
        f"[B]Expires:[/B] {d['expiration']}  ({d['days_left']} days left)\n"
        f"[B]Subscribed:[/B] {'Yes' if d['is_subscribed'] else 'No'}\n"
        f"[B]Total downloaded:[/B] {d['total_downloaded']}\n"
        f"[B]Server:[/B] {d['server']}\n"
        f"[B]Customer ID:[/B] {d['customer']}\n"
        f"[B]Member since:[/B] {d['created_at']}"
    )
    xbmcgui.Dialog().textviewer('TorBox Account', body)


def _tb_torrent_label(item):
    """Build a label line with progress bar / status / size."""
    name = item.get('name', '')[:90]
    size = torbox_advanced._fmt_size(item.get('size', 0))
    state = (item.get('download_state') or item.get('state') or '').lower()
    completed = bool(item.get('download_finished')
                     or item.get('download_present'))
    if completed or state in ('completed', 'cached', 'uploading', 'seeding',
                              'finished', 'uploaded'):
        return f'[COLOR lime][DONE][/COLOR] {name}  [COLOR silver]({size})[/COLOR]'
    bar = torbox_advanced.progress_bar(item.get('progress', 0), width=15)
    status = torbox_advanced.format_torrent_progress(item)
    return (f'[COLOR yellow]{bar}[/COLOR]  {name}  '
            f'[COLOR silver]({size})[/COLOR]\n[COLOR gray]{status}[/COLOR]')


def tb_torrents(filter_state=''):
    tb = torbox_advanced.TorboxAdvanced()
    torrents = tb.torrent_list(bypass_cache=True)
    if not torrents:
        xbmcgui.Dialog().notification('TorBox', 'No torrents in your cloud',
                                      xbmcgui.NOTIFICATION_INFO)
        xbmcplugin.endOfDirectory(HANDLE)
        return

    def is_done(t):
        st = (t.get('download_state') or '').lower()
        return bool(t.get('download_finished')
                    or t.get('download_present')) or st in (
            'completed', 'cached', 'uploading', 'seeding', 'finished', 'uploaded')

    if filter_state == 'active':
        torrents = [t for t in torrents if not is_done(t)]
    elif filter_state == 'done':
        torrents = [t for t in torrents if is_done(t)]

    addon_icon = get_addon_icon()
    addon_fanart = get_addon_fanart()
    for t in torrents:
        tid = t.get('id')
        li = xbmcgui.ListItem(label=_tb_torrent_label(t))
        li.setArt({'icon': addon_icon, 'thumb': addon_icon, 'fanart': addon_fanart})
        if is_done(t):
            li.setProperty('IsPlayable', 'true')
            url = build_url({'action': 'tb_play_torrent', 'torrent_id': tid})
        else:
            url = build_url({'action': 'tb_torrent_status', 'torrent_id': tid})
        # Rich context menu - every TorBox control op
        ctx = [
            ('Play (pick file)', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_play_torrent&torrent_id={tid}&pick=1)"),
            ('Pause',             f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=pause&torrent_id={tid})"),
            ('Resume',            f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=resume&torrent_id={tid})"),
            ('Reannounce',        f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=reannounce&torrent_id={tid})"),
            ('Stop seeding',      f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=stop_seeding&torrent_id={tid})"),
            ('Queue to top',      f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=queue_top&torrent_id={tid})"),
            ('Queue to bottom',   f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=queue_bottom&torrent_id={tid})"),
            ('[COLOR red]Delete[/COLOR]', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_op&op=delete&torrent_id={tid})"),
        ]
...
def tb_torrent_status(torrent_id):
    tb = torbox_advanced.TorboxAdvanced()
    info = tb.torrent_info(torrent_id)
    if not info:
        xbmcgui.Dialog().ok('TorBox', 'Torrent not found')
        return
    body = (
        f"[B]Name:[/B] {info.get('name', '')}\n"
        f"[B]Size:[/B] {torbox_advanced._fmt_size(info.get('size', 0))}\n"
        f"[B]State:[/B] {info.get('download_state', '')}\n"
        f"[B]Progress:[/B] {torbox_advanced.format_torrent_progress(info)}\n"
        f"[B]Ratio:[/B] {info.get('ratio', 0)}\n"
        f"[B]Seeds/Peers:[/B] {info.get('seeds', 0)} / {info.get('peers', 0)}\n"
        f"[B]Hash:[/B] {info.get('hash', '')}\n"
        f"[B]Cached:[/B] {'Yes' if info.get('cached') else 'No'}\n"
        f"[B]Files:[/B] {len(info.get('files') or [])}"
    )
    xbmcgui.Dialog().textviewer('Torrent Status', body)


def tb_op(operation, torrent_id):
    tb = torbox_advanced.TorboxAdvanced()
    if operation == 'delete':
        if not xbmcgui.Dialog().yesno('Delete torrent',
                                      'Permanently remove this torrent from your TorBox cloud?'):
            return
    status, res = tb.torrent_control(torrent_id, operation)
    ok = isinstance(res, dict) and res.get('success')
    msg = f'{operation.replace("_", " ").title()}: {"OK" if ok else "Failed"}'
    xbmcgui.Dialog().notification('TorBox', msg,
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)
    xbmc.executebuiltin('Container.Refresh')


def tb_play_torrent(torrent_id, pick=False):
    tb = torbox_advanced.TorboxAdvanced()
    file_id, name = tb.pick_file_from_torrent(torrent_id, prompt=bool(pick))
    if file_id is None:
        xbmcgui.Dialog().notification('TorBox', 'No playable file found',
                                      xbmcgui.NOTIFICATION_WARNING)
        xbmcplugin.setResolvedUrl(HANDLE, False, xbmcgui.ListItem())
        return
    url = tb.requestdl(torrent_id, file_id)
    if not url:
        xbmcgui.Dialog().ok('TorBox', 'Failed to get download link.\nThe torrent may still be processing.')
        xbmcplugin.setResolvedUrl(HANDLE, False, xbmcgui.ListItem())
        return
    li = xbmcgui.ListItem(label=name or 'TorBox', path=url)
    xbmcplugin.setResolvedUrl(HANDLE, True, li)


def tb_search():
    q = xbmcgui.Dialog().input('Search TorBox')
    if not q:
        return
    tb = torbox_advanced.TorboxAdvanced()
    progress = xbmcgui.DialogProgress()
    progress.create('TorBox Search', f'Searching for "{q}"...')
    results = tb.search(q)
    progress.close()
    if not results:
        xbmcgui.Dialog().ok('TorBox Search',
                            'No results - TorBox search may be unavailable for your plan.')
        return
    # Reuse the source picker for a familiar UI
    from resources.lib import source_picker
    # Check cache so users see CACHED badge for known hashes
    ...
    try:
        ...
        pass
    selected = source_picker.show_source_picker(
        results, cached_set, f'TorBox: {q}', include_free_links=False)
    if not selected:
        return
    magnet = selected.get('magnet')
    if not magnet:
    ...
    if not selected.get('is_cached'):
        choice = xbmcgui.Dialog().select(
            'Source Not Cached',
            [
                '[COLOR yellow]Try to stream now[/COLOR]',
                '[COLOR lime]Queue to TorBox Cloud[/COLOR]',
                '[COLOR red]Cancel[/COLOR]',
            ])
        if choice == 1:
            tb.queue_torrent(magnet, as_queued=True)
            xbmcgui.Dialog().notification('TorBox', 'Queued to cloud',
                                          xbmcgui.NOTIFICATION_INFO)
            return
        if choice != 0:
            return
    progress = xbmcgui.DialogProgress()
    progress.create('TorBox', 'Resolving stream...')
    url, _ = debrid.resolve_magnet(magnet)
    progress.close()
    if url:
        xbmc.Player().play(url)
    else:
        xbmcgui.Dialog().ok('TorBox', 'Could not resolve this source.')


def tb_add_magnet():
    s = xbmcgui.Dialog().input('Paste a magnet link or info hash')
    if not s:
        return
    if not s.startswith('magnet:'):
        ...
        h = s.strip().lower()
        if len(h) in (32, 40):
            s = f'magnet:?xt=urn:btih:{h}'
        else:
            xbmcgui.Dialog().notification('TorBox', 'Not a magnet/hash',
                                          xbmcgui.NOTIFICATION_ERROR)
            return
    tb = torbox_advanced.TorboxAdvanced()
    res = tb.queue_torrent(s, as_queued=False)
    ok = isinstance(res, dict) and res.get('success')
    xbmcgui.Dialog().notification('TorBox',
                                  'Added to cloud' if ok else 'Failed to add',
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)


def _tb_simple_label(item, kind='dl'):
    name = item.get('name', '')[:90]
    size = torbox_advanced._fmt_size(item.get('size', 0))
    state = (item.get('download_state') or item.get('state') or '').lower()
    done = bool(item.get('download_finished') or item.get('download_present')) \
        or state in ('completed', 'finished', 'uploaded')
    tag = '[COLOR lime][DONE][/COLOR]' if done else \
        f'[COLOR yellow]{torbox_advanced.progress_bar(item.get("progress", 0), 15)}[/COLOR]'
    return f'{tag} {name}  [COLOR silver]({size})[/COLOR]'


def tb_usenet_list():
    tb = torbox_advanced.TorboxAdvanced()
    items = tb.usenet_list()
    if not items:
        xbmcgui.Dialog().notification('TorBox', 'No usenet downloads',
                                      xbmcgui.NOTIFICATION_INFO)
        xbmcplugin.endOfDirectory(HANDLE)
        return
    addon_icon = get_addon_icon()
    for it in items:
        uid = it.get('id')
        li = xbmcgui.ListItem(label=_tb_simple_label(it, 'usenet'))
        li.setArt({'icon': addon_icon, 'thumb': addon_icon})
        url = build_url({'action': 'tb_play_usenet', 'usenet_id': uid})
        li.setProperty('IsPlayable', 'true')
        li.addContextMenuItems([
            ('Pause',  f"RunPlugin(plugin://plugin.video.genesis/?action=tb_usenet_op&op=pause&usenet_id={uid})"),
            ('Resume', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_usenet_op&op=resume&usenet_id={uid})"),
            ('[COLOR red]Delete[/COLOR]', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_usenet_op&op=delete&usenet_id={uid})"),
        ])
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=False)
    xbmcplugin.endOfDirectory(HANDLE)
...
def tb_add_nzb():
    s = xbmcgui.Dialog().input('Paste an NZB URL')
    if not s:
        return
    tb = torbox_advanced.TorboxAdvanced()
    res = tb.add_nzb_url(s)
    ok = isinstance(res, dict) and res.get('success')
    xbmcgui.Dialog().notification('TorBox',
                                  'NZB added' if ok else 'Failed',
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)


def tb_usenet_op(operation, usenet_id):
    tb = torbox_advanced.TorboxAdvanced()
    if operation == 'delete' and not xbmcgui.Dialog().yesno('Delete', 'Remove this usenet download?'):
        return
    status, res = tb.usenet_control(usenet_id, operation)
    ok = isinstance(res, dict) and res.get('success')
    xbmcgui.Dialog().notification('TorBox',
                                  f'{operation}: {"OK" if ok else "Failed"}',
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)
    xbmc.executebuiltin('Container.Refresh')


def tb_play_usenet(usenet_id):
    tb = torbox_advanced.TorboxAdvanced()
    url = tb.usenet_requestdl(usenet_id, file_id=0)
    if not url:
        xbmcgui.Dialog().ok('TorBox', 'Could not resolve usenet item.')
        xbmcplugin.setResolvedUrl(HANDLE, False, xbmcgui.ListItem())
        return
    li = xbmcgui.ListItem(path=url)
    xbmcplugin.setResolvedUrl(HANDLE, True, li)
...
def tb_webdl_list():
    ...
    items = tb.webdl_list()
    if not items:
        xbmcgui.Dialog().notification('TorBox', 'No web downloads',
                                      xbmcgui.NOTIFICATION_INFO)
        xbmcplugin.endOfDirectory(HANDLE)
        return
    addon_icon = get_addon_icon()
    for it in items:
        wid = it.get('id')
        li = xbmcgui.ListItem(label=_tb_simple_label(it, 'web'))
        li.setArt({'icon': addon_icon, 'thumb': addon_icon})
        url = build_url({'action': 'tb_play_webdl', 'webdl_id': wid})
        li.setProperty('IsPlayable', 'true')
        li.addContextMenuItems([
            ('Pause',  f"RunPlugin(plugin://plugin.video.genesis/?action=tb_webdl_op&op=pause&webdl_id={wid})"),
            ('Resume', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_webdl_op&op=resume&webdl_id={wid})"),
            ('[COLOR red]Delete[/COLOR]', f"RunPlugin(plugin://plugin.video.genesis/?action=tb_webdl_op&op=delete&webdl_id={wid})"),
        ])
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=False)
    xbmcplugin.endOfDirectory(HANDLE)
...
def tb_add_webdl():
    s = xbmcgui.Dialog().input('Paste a DDL / hoster URL')
    if not s:
        return
    tb = torbox_advanced.TorboxAdvanced()
    res = tb.add_webdl(s)
    ok = isinstance(res, dict) and res.get('success')
    xbmcgui.Dialog().notification('TorBox',
                                  'Web download added' if ok else 'Failed',
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)


def tb_webdl_op(operation, webdl_id):
    tb = torbox_advanced.TorboxAdvanced()
    if operation == 'delete' and not xbmcgui.Dialog().yesno('Delete', 'Remove this web download?'):
        return
    status, res = tb.webdl_control(webdl_id, operation)
    ok = isinstance(res, dict) and res.get('success')
    xbmcgui.Dialog().notification('TorBox',
                                  f'{operation}: {"OK" if ok else "Failed"}',
                                  xbmcgui.NOTIFICATION_INFO if ok else xbmcgui.NOTIFICATION_ERROR)
...
def tb_play_webdl(webdl_id):
    tb = torbox_advanced.TorboxAdvanced()
    url = tb.webdl_requestdl(webdl_id, file_id=0)
    if not url:
        xbmcgui.Dialog().ok('TorBox', 'Could not resolve web download.')
        xbmcplugin.setResolvedUrl(HANDLE, False, xbmcgui.ListItem())
        return
    li = xbmcgui.ListItem(path=url)
    xbmcplugin.setResolvedUrl(HANDLE, True, li)
...
if __name__ == '__main__':
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # PREMIUMIZE HUB ROUTES
    # ══════════════════════════════════════════════════════════════════════════
    elif action == 'pm_hub':
        premiumize_hub.pm_hub()
    elif action == 'pm_account_dashboard':
        premiumize_hub.pm_account_dashboard()
    elif action == 'pm_cloud_folder':
        premiumize_hub.pm_cloud_folder(params.get('folder_id', ''))
    elif action == 'pm_listall':
        premiumize_hub.pm_listall()
    elif action == 'pm_item_details':
        premiumize_hub.pm_item_details(params.get('item_id', ''))
    elif action == 'pm_transfers':
        premiumize_hub.pm_transfers()
    elif action == 'pm_add_transfer_menu':
        premiumize_hub.pm_add_transfer_menu()
    elif action == 'pm_add_magnet':
        premiumize_hub.pm_add_magnet()
    elif action == 'pm_add_url':
        premiumize_hub.pm_add_url()
    elif action == 'pm_add_nzb':
        premiumize_hub.pm_add_nzb()
    elif action == 'pm_delete_transfer':
        premiumize_hub.pm_delete_transfer(params.get('transfer_id', ''))
    elif action == 'pm_clear_finished':
        premiumize_hub.pm_clear_finished()
    elif action == 'pm_create_folder':
        premiumize_hub.pm_create_folder(params.get('parent_id', '0'))
    elif action == 'pm_rename_folder':
        premiumize_hub.pm_rename_folder(params.get('folder_id', ''),
                                        params.get('old_name', ''))
    elif action == 'pm_rename_item':
        premiumize_hub.pm_rename_item(params.get('item_id', ''),
                                      params.get('old_name', ''))
    elif action == 'pm_delete_folder':
        premiumize_hub.pm_delete_folder(params.get('folder_id', ''))
    elif action == 'pm_delete_item':
        premiumize_hub.pm_delete_item(params.get('item_id', ''))
    elif action == 'pm_move_picker':
        premiumize_hub.pm_move_picker(
            item_id_to_move=params.get('item_id_to_move', ''),
            folder_id_to_move=params.get('folder_id_to_move', ''))
    elif action == 'pm_zip_download':
        premiumize_hub.pm_zip_download(item_ids=params.get('item_ids', ''),
                                       folder_ids=params.get('folder_ids', ''))
    elif action == 'pm_services':
        premiumize_hub.pm_services()
    elif action == 'pm_play_link':
        premiumize_hub.pm_play_link(params.get('link', ''),
                                    params.get('item_id', ''))
    elif action == 'pm_play_item':
        premiumize_hub.pm_play_item(params.get('item_id', ''))
    elif action == 'pm_copy_link':
        premiumize_hub.pm_copy_link(params.get('link', ''))
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        tb_hub()
    elif action == 'tb_dashboard':
        tb_dashboard()
    elif action == 'tb_torrents':
        tb_torrents(params.get('filter', ''))
    elif action == 'tb_torrent_status':
        tb_torrent_status(params.get('torrent_id', ''))
    elif action == 'tb_op':
        tb_op(params.get('op', ''), params.get('torrent_id', ''))
    elif action == 'tb_play_torrent':
        tb_play_torrent(params.get('torrent_id', ''), pick=bool(params.get('pick')))
    elif action == 'tb_search':
        tb_search()
    elif action == 'tb_add_magnet':
        tb_add_magnet()
    elif action == 'tb_usenet_list':
        tb_usenet_list()
    elif action == 'tb_add_nzb':
        tb_add_nzb()
    elif action == 'tb_usenet_op':
        tb_usenet_op(params.get('op', ''), params.get('usenet_id', ''))
    elif action == 'tb_play_usenet':
        tb_play_usenet(params.get('usenet_id', ''))
    elif action == 'tb_webdl_list':
        tb_webdl_list()
    elif action == 'tb_add_webdl':
        tb_add_webdl()
    elif action == 'tb_webdl_op':
        tb_webdl_op(params.get('op', ''), params.get('webdl_id', ''))
    elif action == 'tb_play_webdl':
        tb_play_webdl(params.get('webdl_id', ''))