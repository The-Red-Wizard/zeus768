...
class Torbox:
    ...
    def authorize(self):
        ...
        try:
            ...
            api_key = dialog.input('TorBox API Key (paste from torbox.app/settings)')
            if not api_key:
                return False
            ...
            if info and info.get('success'):
                self._save_to_file()
                dialog.notification('Success', 'Torbox linked!', xbmcgui.NOTIFICATION_INFO)
                xbmc.log('Torbox: API key validated and saved', xbmc.LOGINFO)
                return True
            self.token = ''
            dialog.notification('Torbox', 'Invalid API key', xbmcgui.NOTIFICATION_ERROR)
            return False
        except Exception as e:
            xbmc.log(f'Torbox auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Error', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR)
            return False
