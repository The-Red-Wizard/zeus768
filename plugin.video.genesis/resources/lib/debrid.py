...
class Torbox:
    ...
    # --- authorization ----------------------------------------------------
    def authorize(self):
        """Let the user pick how to link TorBox: device-code or manual API key."""
        try:
            dialog = xbmcgui.Dialog()
            choice = dialog.select(
                'Link TorBox',
                ['Get Code (link device at tor.box/link)',
    ...
    def authorize_device(self):
        """Device-code (Get Code) flow.

        Calls GET /v1/api/user/auth/device/start, displays the short user code
        + verification URL in a progress dialog, then polls
        POST /v1/api/user/auth/device/token until the user activates the code.
        """
        dialog = xbmcgui.Dialog()
        progress = None
        try:
            status, result = _get(
                ...
                params={'app': 'Genesis Kodi'},
            )
            if not isinstance(result, dict) or not result.get('success'):
                err = (result or {}).get('detail') if isinstance(result, dict) \
                    else 'Network error'
                dialog.notification('TorBox', f'Cannot start: {err}',
                                    xbmcgui.NOTIFICATION_ERROR)
                return False
            ...
            # Compute deadline (~10 min if not provided)
            ...
            progress.create(
                'TorBox - Link Device',
                'Visit the URL below on a phone or PC,',
                'then enter the code to authorize this device.',
                ' ',
            )
            ...
            while time.time() < deadline:
                ...
                progress.update(
                    min(99, int(poll_count * interval * 100 / max(1, deadline - time.time() + poll_count * interval))),
                    f'URL: [B]{verify_url}[/B]',
                    f'Code: [COLOR yellow][B]{user_code}[/B][/COLOR]',
                    f'Waiting...  expires in {mins:02d}:{secs:02d}',
                )
                ...
                if isinstance(poll, dict) and poll.get('success'):
                    ...
                    if isinstance(pdata, str):
                        ...
                        api_key = (pdata.get('api_key')
                                   ...
                                   or '')
                    if not api_key:
                        dialog.notification('TorBox', 'Auth response missing key',
                                            xbmcgui.NOTIFICATION_ERROR)
                        return False
                    ...
                    if info and info.get('success'):
                        ...
                        dialog.notification('Success', 'TorBox linked!',
                                            xbmcgui.NOTIFICATION_INFO)
                        xbmc.log('Torbox: device-code auth success',
                                 xbmc.LOGINFO)
                        return True
                    self.token = ''
                    dialog.notification('TorBox', 'Key validation failed',
                                        xbmcgui.NOTIFICATION_ERROR)
                    return False

                err = (poll or {}).get('error') if isinstance(poll, dict) else ''
                if err in ('DEVICE_CODE_EXPIRED', 'EXPIRED_TOKEN',
                           'INVALID_DEVICE_CODE'):
                    dialog.notification('TorBox', 'Code expired',
                                        xbmcgui.NOTIFICATION_ERROR)
                    return False

                # DEVICE_CODE_NOT_USED / AUTHORIZATION_PENDING / SLOW_DOWN
                if err == 'SLOW_DOWN':
