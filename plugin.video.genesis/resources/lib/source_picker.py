...
def _send_source_to_pm(src, title=''):
    """Send a single source (magnet/url) to the user's Premiumize cloud."""
    try:
        from resources.lib import premiumize_hub
    except Exception as e:
        xbmc.log(f'Source Picker: premiumize_hub import failed: {e}', xbmc.LOGERROR)
        xbmcgui.Dialog().notification('Premiumize',
                                      'Premiumize hub unavailable',
                                      xbmcgui.NOTIFICATION_ERROR)
        return False
    payload = src.get('magnet') or src.get('url') or src.get('link') or ''
    if not payload:
        xbmcgui.Dialog().notification('Premiumize',
                                      'No magnet / URL on this source',
                                      xbmcgui.NOTIFICATION_WARNING)
        return False
    label = src.get('title') or title or 'source'
    return premiumize_hub.send_to_pm(payload, label[:80])
...
def build_source_label(source, is_cached=False):
    ...
    # Status indicator
    if is_free:
        parts.append('[COLOR magenta][FREE][/COLOR]')
    elif is_cached:
        parts.append('[COLOR lime][CACHED][/COLOR]')
    else:
        parts.append('[COLOR orange][TORRENT][/COLOR]')
...
def show_source_picker(sources, cached_hashes=None, title='', include_free_links=True):
    ...
    filter_options = [
        ...
        '[B][COLOR yellowgreen]★ Send Mode: tap a source to push it to Premiumize Cloud[/COLOR][/B]',
    ]
    ...
    while True:
        ...
        send_to_pm_mode = (filter_choice == 8)

        # Apply filter
        if filter_choice == 0 or send_to_pm_mode:
        ...
        for src in filtered:
            label = build_source_label(src, src['is_cached'])
            if send_to_pm_mode:
                label = '[COLOR yellowgreen][→PM][/COLOR] ' + label
        ...
        if send_to_pm_mode:
            header = (f'[COLOR yellowgreen]>> SEND MODE <<[/COLOR]  '
                      f'tap to push to Premiumize Cloud  •  '
                      f'{len(filtered)} sources  •  '
                      f'[COLOR lime]{cached_count} cached[/COLOR]')
        else:
            header = (f'{len(filtered)} sources  •  '
                      f'[COLOR lime]{cached_count} CACHED[/COLOR]  •  '
                      f'[COLOR magenta]{free_count} FREE[/COLOR]  •  '
                      f'[COLOR orange]TORRENT = not cached[/COLOR]')
        
        # In send mode allow multiple consecutive sends
        while True:
            choice = xbmcgui.Dialog().select(header, filtered_items, useDetails=False)
            if choice < 0:
                if send_to_pm_mode:
                    break  # back to filters
                return None
            if choice == 0:
                # Back to filters
                break
            if send_to_pm_mode:
                src = filtered[choice - 1]
                _send_source_to_pm(src, title)
                # stay in source list for more sends
                continue
            else:
