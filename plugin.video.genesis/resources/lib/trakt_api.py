        ...
        for item in items_data:
            ...
            if media_type == 'movie':
                ...
                li.addContextMenuItems([
                    ('X-Ray: Cast & Info', f'RunPlugin(plugin://plugin.video.genesis/?action=xray&tmdb_id={tmdb_id}&media_type=movie&title={quote_plus(title)}&imdb_id={imdb_id})'),
                    ('Extras: Similar & Cast', f'Container.Update(plugin://plugin.video.genesis/?action=extras_menu&tmdb_id={tmdb_id}&media_type=movie&title={quote_plus(title)}&imdb_id={imdb_id})'),
                ])
                
                play_url = f"{sys.argv[0]}?action=play&title={quote_plus(title)}&year={year}&imdb_id={imdb_id}&tmdb_id={tmdb_id}"
...
def get_calendar():
    """Get upcoming episodes from Trakt calendar with TMDB posters."""
    headers = get_headers()
    ...
    # Batch fetch TMDB images for shows
    images_cache = tmdb.get_images_batch(tmdb_ids, 'tv') if tmdb_ids else {}
    
    for item in items_data:
        label = f'{item["title"]} S{item["season"]:02d}E{item["ep_num"]:02d}'
        if item['ep_title']:
            label += f' - {item["ep_title"]}'
        if item['first_aired']:
            label += f' [{item["first_aired"]}]'
        
        # Get poster from TMDB cache
        images = images_cache.get(item['tmdb_id'], {})
        poster = images.get('poster', '')
        fanart = images.get('backdrop', '') or addon_fanart
        ...
        info_tag.setEpisode(item['ep_num'])
        
        # Add X-Ray context menu
        if item['tmdb_id']:
            li.addContextMenuItems([
                ('X-Ray: Cast & Info', f'RunPlugin(plugin://plugin.video.genesis/?action=xray&tmdb_id={item["tmdb_id"]}&media_type=tv&season={item["season"]}&episode={item["ep_num"]}&title={quote_plus(item["title"])})'),
            ])
        
        url = f"{sys.argv[0]}?action=play_episode&title={quote_plus(item['title'])}&season={item['season']}&episode={item['ep_num']}&imdb_id={item['imdb_id']}&tmdb_id={item['tmdb_id']}"
        li.setProperty('IsPlayable', 'true')
        xbmcplugin.addDirectoryItem(handle, url, li, False)
