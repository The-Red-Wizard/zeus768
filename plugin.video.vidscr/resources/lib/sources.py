...
2. **Multi-link aggregation** — set ``aggregate_all_sources = true``.
   ...
   Slower first-resolve, but the player gets dozens of fallback URLs so
   stall-recovery is near-bulletproof.
...
# Order matters — most reliable mirrors first.
...
def _run_secondary(media_type, tmdb_id, season, episode, imdb_id, host=None):
    """Run vidsrc2.resolve_all, optionally pinning a specific host for the
    duration of the call by overriding the addon setting in-process."""
    addon = xbmcaddon.Addon()
    prev_host = None
    if host:
        try:
            prev_host = addon.getSetting('secondary_source_host')
            addon.setSetting('secondary_source_host', host)
        except Exception:
            prev_host = None
    try:
        ...
        if host and prev_host is not None:
            try:
                addon.setSetting('secondary_source_host', prev_host)
            except Exception:
                pass
    for s in streams:
...
def resolve(media_type, tmdb_id, season=None, episode=None, imdb_id=None,
    ...
    # Multi-link aggregation overrides the sequential flow entirely — we
    # query primary AND every secondary host in parallel, dedupe, return one
    # giant pool of candidates so the player has lots to fall back to.
