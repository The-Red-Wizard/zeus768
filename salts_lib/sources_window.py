...
def strip_bbcode(text):
    """Remove all BBCode formatting tags from text."""
    # Remove [COLOR xxx]...[/COLOR] and other BBCode tags
...
class SaltsSourcePickerWindow(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        # We store both the formatted label (for unfocused) and plain label (for focused).
        try:
            lst = self.getControl(S_LIST)
            li_objs = []
            for label in self._items:
                li = xbmcgui.ListItem(label=label)
                # Store plain version without BBCode for focused state
                plain_label = strip_bbcode(label)
                li.setProperty('plain_label', plain_label)
                li_objs.append(li)
            lst.reset()
