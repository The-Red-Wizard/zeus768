...
BASE_URL = 'https://rlsbb.to'

def is_enabled():
    return control.setting('scraper.rlsbb') == 'true'
...
def _search(query):
    ...
    try:
        ...
        if not html:
            return results
        posts = re.findall(r'<h\d[^>]*class="[^"]*postTitle[^"]*"[^>]*>\s*<a[^>]*href="([^"]*)"[^>]*>([^<]*)</a>', html)
        if not posts:
            posts = re.findall(r'<a[^>]*href="(%s/[^"]*)"[^>]*rel="bookmark"[^>]*>([^<]*)</a>' % re.escape(BASE_URL), html)
        for url, name in posts:
            results.append((url, name.strip()))
    except Exception as e:
        control.log('RLSbb search error: %s' % e)
    return results
...
def _get_links(url):
    ...
    try:
        ...
        if not html:
            return links
        ddl_patterns = [
            r'href="(https?://(?:rapidrar|nitroflare|clicknupload|uploadgig|rapidgator|turbobit|uploaded|ddownload|katfile|mega|1fichier|hexupload|filefactory|oboom)[^"]*)"',
        ]
    ...
    try:
        ...
        for url, name in posts[:10]:
            ...
            for link_url in links:
                ...
                label = '[COLOR red]RLSbb[/COLOR] | %s | %s | %s' % (quality, info, hoster) if info else '[COLOR red]RLSbb[/COLOR] | %s | %s' % (quality, hoster)
                sources.append({
                    'source': 'RLSbb', 'quality': quality, 'info': info,
                    'label': label, 'url': link_url, 'name': name,
                    'type': 'torrent' if is_magnet else 'hoster', 'debrid': '', 'direct': False,
                })
    except Exception as e:
        control.log('RLSbb movie error: %s' % e)
    return sources
...
def get_episode_sources(title, season, episode, imdb=''):
    ...
    try:
        ...
        control.log('RLSbb episode error: %s' % e)
    return sources
