...
class Torbox:
    ...
    def unrestrict_link(self, link):
        ...
        try:
            ...
            # Wait for the web download to finish (cached links resolve fast)
            file_id = 0
            for _ in range(30):
                lst_status, lst_result = _get(
                ...
                if isinstance(lst_result, dict) and lst_result.get('success'):
                    ...
                    if finished or state in ('completed', 'cached',
                                             'uploading', 'seeding'):
                        if files:
    ...
    def add_magnet(self, magnet, check_cache_first=True):
        ...
        try:
            ...
            # Wait for torrent to be ready and pick the best video file
            for _ in range(30):
                ...
                if not (isinstance(list_result, dict) and list_result.get('success')):
                    time.sleep(2)
                    continue

                torrent_data = list_result.get('data', {}) or {}
                if isinstance(torrent_data, list) and torrent_data:
                    torrent_data = torrent_data[0]

                state = (torrent_data.get('download_state') or '').lower()
                finished = torrent_data.get('download_finished')
                ...
                if finished or state in ('completed', 'cached',
                    ...
                    if not picked:
                        return None
                    return self.request_torrent_link(torrent_id, picked['id'])

                if state in ('error', 'failed'):
                    xbmc.log(f'Torbox: Torrent failed - {state}',
                             xbmc.LOGERROR)
                    return None
