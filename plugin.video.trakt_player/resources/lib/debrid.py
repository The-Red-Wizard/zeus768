...
_VIDEO_EXTS = ('.mkv', '.mp4', '.avi', '.mov', '.wmv', '.m4v', '.webm',
               '.flv', '.ts', '.m2ts', '.mpg', '.mpeg', '.iso', '.vob')
_EXTRAS_FILTER = ('sample', 'extra', 'extras', 'deleted', 'unused',
                  'footage', 'inside', 'blooper', 'bloopers', 'making.of',
                  'feature', 'featurette', 'behind.the.scenes', 'trailer')


def _tb_pick_video_file(files, prefer_id=None):
    """Pick the best video file from a Torbox files list.
    Returns dict with id, name, size or None if no video found.
    """
    if not files:
        return None
    candidates = []
    ...
    if not candidates:
        return None
    # Filter out obvious extras when more than one candidate
    if len(candidates) > 1:
        filtered = [c for c in candidates
                    if not any(tag in c['name'].lower().replace(' ', '.')
                               for tag in _EXTRAS_FILTER)]
        if filtered:
            candidates = filtered
    if prefer_id is not None:
        for c in candidates:
            if str(c['id']) == str(prefer_id):
                return c
    candidates.sort(key=lambda k: k['size'], reverse=True)
    return candidates[0]


class Torbox:
    """Torbox API integration with device code authentication.
    Supports torrents, usenet (NZB), and web downloads with full cloud
    browsing, cached lookups (single + batch), and smart file selection.
    """
    ...
    def _load_from_file(self):
        """Load token from file. Settings value (if present) wins, so that
        a user pasting their API key in the addon settings is honored
        immediately on the next instantiation.
        """
        _ensure_data_path()
        self.token = ''

        # 1) Addon setting takes priority (user might have just pasted it)
        try:
            settings_token = (get_addon().getSetting('tb_api_key') or '').strip()
        except Exception:
            settings_token = ''
        ...
        if os.path.exists(TB_TOKEN_FILE):
            try:
                with open(TB_TOKEN_FILE, 'r') as f:
                    data = json.load(f)
                    file_token = (data.get('access_token') or '').strip()
            except Exception:
                pass

        self.token = settings_token or file_token
        if self.token:
            xbmc.log('Torbox: Loaded token', xbmc.LOGDEBUG)

        # Keep file + settings in sync so both code paths see the key
        if self.token and self.token != file_token:
            try:
                self._save_to_file()
            except Exception:
                pass
    ...
    def verify_token(self):
        """Hit /user/me to verify the current token works."""
        if not self.token:
            return False
        try:
            ...
            if status == 200 and isinstance(result, dict) and result.get('success'):
                return True
            xbmc.log(f'Torbox: token verify failed - status={status} '
                     f'result={result}', xbmc.LOGWARNING)
        except Exception as e:
            xbmc.log(f'Torbox verify_token error: {e}', xbmc.LOGERROR)
        return False

    def authorize_api_key(self, key=None):
        """Authorize Torbox using a manually-entered API key.

        Prompts the user for the key if none is supplied, then validates it
        against /user/me before persisting.
        """
        try:
            if not key:
                dialog = xbmcgui.Dialog()
                key = dialog.input('Enter your TorBox API Key',
                                   defaultt=self.token or '')
                if not key:
                    return False
            key = key.strip()
            if not key:
            ...
            self._save_to_file()
            xbmcgui.Dialog().notification(
                'TorBox', 'API key saved & verified',
                xbmcgui.NOTIFICATION_INFO
            )
            xbmc.log('Torbox: API key authorization successful',
                     xbmc.LOGINFO)
            return True
        except Exception as e:
            xbmc.log(f'Torbox authorize_api_key error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification(
                'TorBox', f'Auth failed: {e}',
    ...
    def authorize(self):
        ...
        try:
            ...
            # TorBox returns user code as 'code', not 'user_code'
            user_code = data.get('code') or data.get('user_code')
            # Prefer friendly (shorter) URL for TV screens
            ...
            while token_ttl > 0:
                ...
                # Step 2: Poll for token (TorBox requires JSON POST)
                check_status, check_result = _post(
    ...
    def check_cache(self, hashes):
        ...
        Returns dict {hash_lower: True} for cached items.
        """
        if not self.is_authorized() or not hashes:
            return {}
        ...
        cached = {}
        try:
            # Torbox supports GET /torrents/checkcached?hash=...&format=list
            # for batches. Process up to 100 hashes per call.
            for i in range(0, len(hashes), 100):
                batch = hashes[i:i + 100]
                params = [('format', 'list')]
                for h in batch:
                    params.append(('hash', h.lower()))
                query = urlencode(params)
                url = f"{self.BASE_URL}/torrents/checkcached?{query}"
                status, result = _get(url, headers=self._auth_headers())
                ...
                if isinstance(data, list):
                    ...
                    for h_key, val in data.items():
                        if val:
                            cached[h_key.lower()] = True
        except Exception as e:
            xbmc.log(f'Torbox cache check error: {e}', xbmc.LOGERROR)
        return cached

    def check_cache_single(self, info_hash):
        """Check a single hash. Returns True/False."""
        if not info_hash:
            return False
        result = self.check_cache([info_hash])
        return bool(result.get(info_hash.lower()))

    def unrestrict_link(self, link):
        """Add a web download (URL) and return a streamable link."""
        if not self.is_authorized():
            return None
        try:
            status, result = _post(
            ...
            if not (isinstance(result, dict) and result.get('success')):
                xbmc.log(f'Torbox webdl create failed - {result}', xbmc.LOGERROR)
                return None
            ...
            # Poll up to ~90s. Cached web downloads resolve almost
            # instantly; fresh ones may take longer.
            file_id = 0
            deadline = time.time() + 90
            while time.time() < deadline:
                _, lst_result = _get(
                    f"{self.BASE_URL}/webdl/mylist",
                    params={"id": download_id, "bypass_cache": "true"},
                    headers=self._auth_headers()
                )
                if isinstance(lst_result, dict) and lst_result.get('success'):
                    info = lst_result.get('data') or {}
                    if isinstance(info, list) and info:
                        info = info[0]
                    state = (info.get('download_state') or '').lower()
                    finished = info.get('download_finished')
                    present = info.get('download_present')
                    files = info.get('files') or []
                    if (finished or present
                            or state in ('completed', 'cached',
                                         'uploading', 'seeding')) and files:
                        picked = _tb_pick_video_file(files) or files[0]
                        file_id = picked.get('id', 0)
                        break
                    if state in ('error', 'failed', 'dead'):
                        return None
                time.sleep(2)
    ...
    def request_torrent_link(self, torrent_id, file_id):
        """Get a streamable link for a Torbox torrent file."""
        if not self.is_authorized():
            return None
        try:
            status, result = _get(
            ...
            xbmc.log(f'Torbox torrent dl error: {e}', xbmc.LOGERROR)
        return None

    def request_usenet_link(self, usenet_id, file_id):
        """Get a streamable link for a Torbox usenet file."""
        if not self.is_authorized():
            return None
        try:
            status, result = _get(
            ...
            xbmc.log(f'Torbox usenet dl error: {e}', xbmc.LOGERROR)
        return None

    def request_webdl_link(self, web_id, file_id):
        """Get a streamable link for a Torbox web-download file."""
        if not self.is_authorized():
            return None
        try:
            status, result = _get(
            ...
            xbmc.log(f'Torbox webdl dl error: {e}', xbmc.LOGERROR)
        return None

    def add_magnet(self, magnet, check_cache_first=True):
        """Add a magnet and return a streamable link to the best video file.

        Strategy:
          1. Extract info hash and (optionally) check Torbox cache first.
          2. POST /torrents/createtorrent. TorBox returns ``torrent_id`` for
             both freshly queued and already-cached torrents, including the
             friendly "Found Cached Torrent" path.
          3. Poll /torrents/mylist until ``download_present`` flips true (or
             state becomes completed/cached/seeding/uploading). A
             ``DialogProgressBG`` keeps the user in the loop.
          4. Pick the best video file with ``_tb_pick_video_file`` and
             return its streamable URL from /torrents/requestdl.
        """
        if not self.is_authorized():
            return None

        # ── 1. info hash + cache check ─────────────────────────────────
        import re
        info_hash = None
        m = re.search(r'btih:([a-fA-F0-9]{40})', magnet or '')
        if not m:
            m = re.search(r'btih:([a-fA-F0-9]{32})', magnet or '')
        if m:
        ...
        if check_cache_first and info_hash:
            try:
                is_cached = self.check_cache_single(info_hash)
                xbmc.log(f'Torbox: cache lookup for {info_hash} -> '
                         f'{is_cached}', xbmc.LOGINFO)
            except Exception:
                pass
        ...
        try:
            ...
            xbmc.log(f'Torbox: createtorrent response - {result}',
                     xbmc.LOGDEBUG)
            ...
            xbmc.log(f'Torbox createtorrent error: {e}', xbmc.LOGERROR)

        # If creation didn't return an id (e.g. duplicate w/o data) try to
        # locate the torrent in mylist by hash.
        if not torrent_id and info_hash:
            try:
        ...
        if not torrent_id:
            xbmc.log('Torbox: could not obtain torrent_id after addMagnet',
                     xbmc.LOGERROR)
            return None

        # ── 3. poll until ready ───────────────────────────────────────
        # Cached torrents resolve in <5s. Uncached ones may take a while —
        # give them up to ~3 minutes before bailing so playback flow has
        # a fair shot before falling back to other debrids.
        max_wait_sec = 30 if is_cached else 180
        ...
        try:
            try:
                ...
                bg.create('TorBox',
                          'Cached - preparing link...' if is_cached
                          else 'Queued on TorBox...')
            except Exception:
                bg = None
            ...
            while time.time() < deadline:
                ...
                if isinstance(list_result, dict) and list_result.get('success'):
                    td = list_result.get('data') or {}
                    if isinstance(td, list) and td:
                        td = td[0]
                    if isinstance(td, dict) and td:
                        torrent_data = td
                        state = (td.get('download_state') or '').lower()
                        finished = bool(td.get('download_finished'))
                        present = bool(td.get('download_present'))
                        cached_flag = bool(td.get('cached'))
                        progress = td.get('progress') or 0
                        files = td.get('files') or []

                        # Ready signals - we can stream
                        ready = (
                            finished or present or cached_flag
                            or state in ('completed', 'cached',
                                         'uploading', 'seeding')
                        )
                        if ready and files:
                        ...
                        if state in ('error', 'failed', 'dead'):
                            xbmc.log(f'Torbox: torrent failed - state={state}',
                                     xbmc.LOGERROR)
                            return None

                        if bg is not None:
                            try:
                                pct = int(progress * 100) if 0 < progress <= 1 \
                                    else int(progress)
                                if pct != last_progress:
                                    bg.update(
                                        max(0, min(100, pct)),
                                        'TorBox',
                                        f'{state or "queued"} - {pct}%'
                                    )
                                    last_progress = pct
                            except Exception:
                                pass
                ...
                xbmc.log('Torbox: polling timed out before torrent was ready',
                         xbmc.LOGWARNING)
                return None
        finally:
            if bg is not None:
        ...
        # ── 4. pick best file and request stream link ─────────────────
        files = torrent_data.get('files') or []
        picked = _tb_pick_video_file(files)
        if not picked and files:
            picked = {'id': files[0].get('id', 0)}
        if not picked:
            xbmc.log('Torbox: no playable file in torrent', xbmc.LOGERROR)
            return None

        link = self.request_torrent_link(torrent_id, picked['id'])
        if not link:
            xbmc.log('Torbox: requestdl returned no link', xbmc.LOGERROR)
        return link

    def add_webdl(self, link):
        """Submit a web URL to Torbox and return its id."""
        if not self.is_authorized():
            return None
        try:
            status, result = _post(
            ...
            if isinstance(result, dict) and result.get('success'):
                data = result.get('data') or {}
                return data.get('webdownload_id') or data.get('id')
        except Exception as e:
            xbmc.log(f'Torbox add_webdl error: {e}', xbmc.LOGERROR)
        return None

    def add_nzb(self, link, name=None):
        """Submit a Usenet NZB URL to Torbox."""
        if not self.is_authorized():
            return None
        try:
            data = {"link": link}
            if name:
                data["name"] = name
            status, result = _post(
                f"{self.BASE_URL}/usenet/createusenetdownload",
                headers=self._auth_headers(),
                data=data
            )
            if isinstance(result, dict) and result.get('success'):
                d = result.get('data') or {}
                return d.get('usenetdownload_id') or d.get('id')
        except Exception as e:
            xbmc.log(f'Torbox add_nzb error: {e}', xbmc.LOGERROR)
        return None
    ...
    def list_torrents(self):
        ...
        try:
            ...
            xbmc.log(f'Torbox list_torrents error: {e}', xbmc.LOGERROR)
        return []
    ...
    def list_usenet(self):
        ...
        try:
            ...
            xbmc.log(f'Torbox list_usenet error: {e}', xbmc.LOGERROR)
        return []
    ...
    def list_webdl(self):
        ...
        try:
            ...
            xbmc.log(f'Torbox list_webdl error: {e}', xbmc.LOGERROR)
        return []
    ...
    def torrent_info(self, torrent_id):
        ...
        try:
            ...
            xbmc.log(f'Torbox torrent_info error: {e}', xbmc.LOGERROR)
        return {}
    ...
    def usenet_info(self, usenet_id):
        ...
        try:
            ...
            xbmc.log(f'Torbox usenet_info error: {e}', xbmc.LOGERROR)
        return {}

    def webdl_info(self, web_id):
        if not self.is_authorized():
            return {}
        try:
            status, result = _get(
            ...
            xbmc.log(f'Torbox webdl_info error: {e}', xbmc.LOGERROR)
        return {}
    ...
    def delete_torrent(self, torrent_id):
        try:
            ...
            xbmc.log(f'Torbox delete_torrent error: {e}', xbmc.LOGERROR)
            return False
    ...
    def delete_usenet(self, usenet_id):
        try:
            ...
            xbmc.log(f'Torbox delete_usenet error: {e}', xbmc.LOGERROR)
            return False

    def delete_webdl(self, web_id):
        try:
            status, result = _post(
                f"{self.BASE_URL}/webdl/controlwebdownload",
                headers=self._auth_headers(),
                data={"webdownload_id": web_id, "operation": "delete"}
            )
            return isinstance(result, dict) and result.get('success')
        except Exception as e:
            xbmc.log(f'Torbox delete_webdl error: {e}', xbmc.LOGERROR)
            return False
...
def _build_tb_items(raw_items, service_kind):
    """Normalize Torbox list entries (torrents/usenet/webdl) into UI items."""
    items = []
    for item in raw_items:
        item_id = item.get('id', '')
        name = item.get('name') or item.get('filename') or 'Unknown'
        state = (item.get('download_state') or '').lower()
        finished = item.get('download_finished')
        progress = item.get('progress', 0) or 0
        size = int(item.get('size', 0) or 0)
        if finished:
            status_str = 'completed'
        else:
            status_str = state or 'queued'
        items.append({
            'id': item_id,
            'name': name,
            'status': status_str,
            'progress': int(progress * 100) if 0 < progress <= 1 else int(progress),
...
def tb_get_torrents():
    ...
    if not tb.is_authorized():
        return []
    return _build_tb_items(tb.list_torrents(), 'torrent')


def tb_get_usenet():
    """Get usenet downloads from Torbox cloud"""
    tb = Torbox()
    if not tb.is_authorized():
        return []
    return _build_tb_items(tb.list_usenet(), 'usenet')


def tb_get_webdl():
    """Get web downloads from Torbox cloud"""
    tb = Torbox()
    if not tb.is_authorized():
        return []
    return _build_tb_items(tb.list_webdl(), 'webdl')


def _tb_files_from(data):
    """Normalize a Torbox item file list into UI items."""
    files = []
    for f in (data.get('files') or []):
        file_id = f.get('id', 0)
        name = (f.get('short_name') or f.get('name') or '').split('/')[-1]
        if not name:
            name = f"File {file_id}"
        size = int(f.get('size', 0) or 0)
...
def tb_get_torrent_files(torrent_id):
    ...
    if not tb.is_authorized():
        return []
    files = _tb_files_from(tb.torrent_info(torrent_id))
    for f in files:
        f['torrent_id'] = torrent_id
    return files


def tb_get_usenet_files(usenet_id):
    """Get files inside a Torbox usenet item"""
    tb = Torbox()
    if not tb.is_authorized():
        return []
    files = _tb_files_from(tb.usenet_info(usenet_id))
    for f in files:
        f['usenet_id'] = usenet_id
    return files


def tb_get_webdl_files(web_id):
    """Get files inside a Torbox web download"""
    tb = Torbox()
    if not tb.is_authorized():
        return []
    files = _tb_files_from(tb.webdl_info(web_id))
    for f in files:
        f['web_id'] = web_id
    return files
...
def tb_get_download_link(torrent_id, file_id):
    """Get download link for a Torbox torrent file"""
    tb = Torbox()
    if not tb.is_authorized():
        return None
    return tb.request_torrent_link(torrent_id, file_id)


def tb_get_usenet_link(usenet_id, file_id):
    """Get download link for a Torbox usenet file"""
    tb = Torbox()
    if not tb.is_authorized():
        return None
    return tb.request_usenet_link(usenet_id, file_id)


def tb_get_webdl_link(web_id, file_id):
    """Get download link for a Torbox web-download file"""
    tb = Torbox()
    if not tb.is_authorized():
        return None
    return tb.request_webdl_link(web_id, file_id)
...
def delete_cloud_item(service, item_id):
    ...
    try:
        if service == 'rd':
            ...
            tb = Torbox()
            if tb.is_authorized():
                return tb.delete_torrent(item_id)

        elif service == 'tb_usenet':
            tb = Torbox()
            if tb.is_authorized():
                return tb.delete_usenet(item_id)

        elif service == 'tb_webdl':
            tb = Torbox()
            if tb.is_authorized():
                return tb.delete_webdl(item_id)
