...
def tb_add_magnet():
    """Prompt for a magnet URL and submit it to Torbox."""
    tb = debrid.Torbox()
    if not tb.is_authorized():
        xbmcgui.Dialog().notification('Torbox', 'Authorize Torbox first',
                                      xbmcgui.NOTIFICATION_WARNING, 3000)
    ...
    if not magnet.startswith('magnet:'):
        xbmcgui.Dialog().notification('Torbox', 'Invalid magnet link',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)
        return
    xbmcgui.Dialog().notification('Torbox', 'Adding magnet...',
                                  xbmcgui.NOTIFICATION_INFO, 2000)
    url = tb.add_magnet(magnet)
    if url:
        xbmcgui.Dialog().notification('Torbox', 'Added and ready',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        xbmc.executebuiltin('Container.Refresh')
    else:
        xbmcgui.Dialog().notification('Torbox', 'Failed to add magnet',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)
