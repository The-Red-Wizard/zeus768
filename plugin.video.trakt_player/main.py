...
from resources.lib import gofile as gofile_lib
...
def cloud_service_menu(service):
    ...
    if service == 'rd':
        ...
        # Torbox: Show torrents, usenet, web downloads, plus add actions
        items.append(_menu_item('[COLOR cyan]My Torrents[/COLOR]', 'cloud_tb_torrents'))
        items.append(_menu_item('[COLOR cyan]My Usenet Downloads[/COLOR]', 'cloud_tb_usenet'))
        items.append(_menu_item('[COLOR cyan]My Web Downloads[/COLOR]', 'cloud_tb_webdl'))
        items.append(_menu_item('[COLOR yellow]Add Web Download[/COLOR]', 'tb_add_webdl', is_folder=False))
        items.append(_menu_item('[COLOR yellow]Add Usenet NZB URL[/COLOR]', 'tb_add_nzb', is_folder=False))
        items.append(_menu_item('[COLOR yellow]Add Magnet[/COLOR]', 'tb_add_magnet', is_folder=False))
...
def cloud_tb_torrent_files(torrent_id):
    ...
    for f in files:
        ...
        # Right-click -> upload this file's resolved link to Gofile
        gofile_action = (
            'RunPlugin(' + build_url({
                'action': 'gofile_upload_tb_torrent',
                'torrent_id': str(torrent_id),
                'file_id': str(f['id']),
            }) + ')'
        )
        li.addContextMenuItems(
            [('Upload to Gofile', gofile_action)], replaceItems=False
        )
...
def cloud_tb_usenet():
    """List Torbox usenet downloads"""
    items = debrid.tb_get_usenet()
    if not items:
    ...
    for item in items:
        ...
        if status in ['completed', 'cached']:
            ...
            label = f"[COLOR gray]{status}[/COLOR] {item['name']}"
        url = build_url({'action': 'cloud_tb_usenet_files', 'usenet_id': item['id']})
        li = xbmcgui.ListItem(label=label)
        li.setArt({'icon': get_addon_icon(), 'thumb': get_addon_icon()})
        li.addContextMenuItems([
            ('Delete', f"RunPlugin(plugin://plugin.video.trakt_player/?action=cloud_delete&service=tb_usenet&item_id={item['id']})")
        ])
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=True)
    xbmcplugin.endOfDirectory(HANDLE)


def cloud_tb_usenet_files(usenet_id):
    """List files in a Torbox usenet item"""
    files = debrid.tb_get_usenet_files(usenet_id)
    if not files:
        xbmcgui.Dialog().notification('Torbox', 'No files found',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        return
    for f in files:
        ...
        if f.get('size'):
            label += f" [{f['size']}]"
        url = build_url({'action': 'cloud_play_tb_usenet',
                         'usenet_id': usenet_id, 'file_id': str(f['id'])})
        li = xbmcgui.ListItem(label=label)
        li.setArt({'icon': get_addon_icon(), 'thumb': get_addon_icon()})
        if f.get('is_video'):
            li.setProperty('IsPlayable', 'true')
...
def cloud_tb_webdl():
    """List Torbox web downloads"""
    items = debrid.tb_get_webdl()
    if not items:
    ...
    for item in items:
        ...
        if status in ['completed', 'cached']:
            ...
            label = f"[COLOR gray]{status}[/COLOR] {item['name']}"
        url = build_url({'action': 'cloud_tb_webdl_files', 'web_id': item['id']})
        li = xbmcgui.ListItem(label=label)
        li.setArt({'icon': get_addon_icon(), 'thumb': get_addon_icon()})
        li.addContextMenuItems([
            ('Delete', f"RunPlugin(plugin://plugin.video.trakt_player/?action=cloud_delete&service=tb_webdl&item_id={item['id']})")
        ])
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=True)
    xbmcplugin.endOfDirectory(HANDLE)


def cloud_tb_webdl_files(web_id):
    """List files in a Torbox web download"""
    files = debrid.tb_get_webdl_files(web_id)
    if not files:
        xbmcgui.Dialog().notification('Torbox', 'No files found',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        return
    for f in files:
        ...
        if f.get('size'):
            label += f" [{f['size']}]"
        url = build_url({'action': 'cloud_play_tb_webdl',
                         'web_id': web_id, 'file_id': str(f['id'])})
        li = xbmcgui.ListItem(label=label)
        li.setArt({'icon': get_addon_icon(), 'thumb': get_addon_icon()})
        if f.get('is_video'):
            li.setProperty('IsPlayable', 'true')
...
def tb_add_webdl():
    """Prompt for a URL and submit it as a Torbox web download."""
    tb = debrid.Torbox()
    if not tb.is_authorized():
        xbmcgui.Dialog().notification('Torbox', 'Authorize Torbox first',
                                      xbmcgui.NOTIFICATION_WARNING, 3000)
        return
    link = xbmcgui.Dialog().input('Web Download URL')
    if not link:
        return
    new_id = tb.add_webdl(link)
    if new_id:
        xbmcgui.Dialog().notification('Torbox', 'Web download queued',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        xbmc.executebuiltin('Container.Refresh')
    else:
        xbmcgui.Dialog().notification('Torbox', 'Failed to add web download',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)


def tb_add_nzb():
    """Prompt for an NZB URL and submit it as a Torbox usenet download."""
    tb = debrid.Torbox()
    if not tb.is_authorized():
        xbmcgui.Dialog().notification('Torbox', 'Authorize Torbox first',
                                      xbmcgui.NOTIFICATION_WARNING, 3000)
        return
    link = xbmcgui.Dialog().input('NZB URL')
    if not link:
        return
    name = xbmcgui.Dialog().input('Name (optional)') or None
    new_id = tb.add_nzb(link, name=name)
    if new_id:
        xbmcgui.Dialog().notification('Torbox', 'NZB download queued',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        xbmc.executebuiltin('Container.Refresh')
    else:
        xbmcgui.Dialog().notification('Torbox', 'Failed to add NZB',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)


def tb_add_magnet():
    """Prompt for a magnet URL, resolve it via Torbox and optionally
    re-upload the resulting stream to Gofile.
    """
    tb = debrid.Torbox()
    if not tb.is_authorized():
        xbmcgui.Dialog().notification('Torbox', 'Authorize Torbox first',
                                      xbmcgui.NOTIFICATION_WARNING, 3000)
        return
    magnet = xbmcgui.Dialog().input('Magnet URI')
    if not magnet:
        return
    if not magnet.startswith('magnet:'):
        xbmcgui.Dialog().notification('Torbox', 'Invalid magnet link',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)
        return
    xbmcgui.Dialog().notification('Torbox', 'Resolving magnet...',
                                  xbmcgui.NOTIFICATION_INFO, 2000)
    url = tb.add_magnet(magnet)
    if not url:
        xbmcgui.Dialog().notification('Torbox', 'Failed to resolve magnet',
                                      xbmcgui.NOTIFICATION_ERROR, 4000)
        return

    xbmcgui.Dialog().notification('Torbox', 'Stream link ready',
                                  xbmcgui.NOTIFICATION_INFO, 3000)

    # Offer Gofile re-upload (toggleable in settings)
    prompt_setting = (get_addon().getSetting('tb_gofile_prompt') or 'true')
    if prompt_setting.lower() == 'true':
        if xbmcgui.Dialog().yesno(
            'Upload to Gofile?',
            'Re-upload this file to Gofile.io (anonymous)?'
        ):
            gofile_lib.upload_and_notify(url)

    xbmc.executebuiltin('Container.Refresh')


def gofile_upload_url(src_url):
    """Action: upload an arbitrary resolved URL to Gofile."""
    if not src_url:
        xbmcgui.Dialog().notification('Gofile', 'No source URL provided',
                                      xbmcgui.NOTIFICATION_ERROR, 3000)
        return
    gofile_lib.upload_and_notify(src_url)


def gofile_upload_tb_torrent(torrent_id, file_id):
    """Resolve a Torbox cloud file then upload it to Gofile."""
    if not torrent_id or not str(file_id):
        return
    direct_url = debrid.tb_get_download_link(torrent_id, int(file_id))
    if not direct_url:
        xbmcgui.Dialog().notification(
            'Torbox', 'Could not resolve link for Gofile upload',
            xbmcgui.NOTIFICATION_ERROR, 3000
        )
        return
    gofile_lib.upload_and_notify(direct_url)


def gofile_copy_last():
    """Action: show / copy the last Gofile link saved in settings."""
    link = (get_addon().getSetting('tb_gofile_last') or '').strip()
    if not link:
        xbmcgui.Dialog().notification('Gofile', 'No saved link yet',
                                      xbmcgui.NOTIFICATION_INFO, 3000)
        return
    # Use input dialog so the user can highlight/copy on TV remotes / keyboards
    xbmcgui.Dialog().input('Last Gofile link (highlight & copy)',
                           defaultt=link)


def auth_tb_apikey():
    """Action: enter Torbox API key manually."""
    debrid.Torbox().authorize_api_key()


def cloud_play_tb_usenet(usenet_id, file_id):
    """Play a Torbox usenet file"""
    direct_url = debrid.tb_get_usenet_link(usenet_id, int(file_id))
    if direct_url:
...
def cloud_play_tb_webdl(web_id, file_id):
    """Play a Torbox web-download file"""
    direct_url = debrid.tb_get_webdl_link(web_id, int(file_id))
    if direct_url:
        li = xbmcgui.ListItem(path=direct_url)
        xbmcplugin.setResolvedUrl(HANDLE, True, li)
    else:
...
if __name__ == '__main__':
    ...
    # Debrid Cloud Browser
    ...
    elif action == 'cloud_tb_usenet':
        cloud_tb_usenet()
    elif action == 'cloud_tb_usenet_files':
        cloud_tb_usenet_files(params.get('usenet_id', ''))
    elif action == 'cloud_tb_webdl':
        cloud_tb_webdl()
    elif action == 'cloud_tb_webdl_files':
        cloud_tb_webdl_files(params.get('web_id', ''))
        ...
        tb_add_magnet()
    elif action == 'gofile_upload':
        gofile_upload_url(params.get('url', ''))
    elif action == 'gofile_upload_tb_torrent':
        gofile_upload_tb_torrent(params.get('torrent_id', ''), params.get('file_id', '0'))
    elif action == 'gofile_copy_last':
        gofile_copy_last()
    elif action == 'cloud_play_tb_usenet':
        cloud_play_tb_usenet(params.get('usenet_id', ''), params.get('file_id', '0'))
    elif action == 'cloud_play_tb_webdl':
        cloud_play_tb_webdl(params.get('web_id', ''), params.get('file_id', '0'))
