...
def main():
    ...
    if not action:
        ...
        items = [
            ...
            ('In Theatres', 'list_content', 'm_now_playing.png', {'type': 'movie', 'sort': 'now_playing'}),
            ('Upcoming', 'list_content', 'm_upcoming.png', {'type': 'movie', 'sort': 'upcoming'}),
            ('Top Rated', 'list_content', 'm_top_rated.png', {'type': 'movie', 'sort': 'top_rated'}),
            ('Documentaries', 'list_content', 'm_documentary.png', {'type': 'movie', 'sort': 'popular', 'genre_id': '99'}),
            ('Anime', 'list_content', 'm_anime.png', {'type': 'movie', 'sort': 'popular', 'genre_id': '16', 'with_keywords': '210024'}),
            ('Networks', 'movie_providers', 't_nets.png'),
        ...
        items = [
            ...
            ('Airing Today', 'list_content', 't_airing_today.png', {'type': 'tv', 'sort': 'airing_today'}),
            ('On The Air', 'list_content', 't_on_the_air.png', {'type': 'tv', 'sort': 'on_the_air'}),
            ('Top Rated', 'list_content', 'm_top_rated.png', {'type': 'tv', 'sort': 'top_rated'}),
            ('Documentaries', 'list_content', 'm_documentary.png', {'type': 'tv', 'sort': 'popular', 'genre_id': '99'}),
            ('Anime', 'list_content', 'm_anime.png', {'type': 'tv', 'sort': 'popular', 'genre_id': '16', 'with_keywords': '210024'}),
        ...
        # Torbox status
        if debrid_manager.tb_authorized():
            tb_user = ADDON.getSetting('tb.username') or 'Authorized'
            items.append((f'[COLOR green]Torbox: {tb_user}[/COLOR]', 'tb_auth', 'tools.png'))
        else:
            items.append(('[COLOR red]Torbox: Not Authorized[/COLOR]', 'tb_auth', 'tools.png'))
        ...
        if debrid_manager.tb_authorized():
            choice = xbmcgui.Dialog().yesno(
                'Torbox',
                f'Currently authorized as: {ADDON.getSetting("tb.username")}\n\nDo you want to re-authorize?'
            )
            if not choice:
                return

        xbmc.log('[Tinklepad] Starting Torbox authorization...', xbmc.LOGINFO)
        success = debrid_manager.tb_authorize()

        if success:
            xbmcgui.Dialog().ok('Torbox', 'Authorization successful!\n\nYou can now use Torbox for magnet/torrent sources.')
        xbmc.executebuiltin('Container.Refresh')
        ...
        info = (
            "[B][COLOR gold]TINKLEPAD v1.9.4[/COLOR][/B]\n\n"
    ...
    elif action == 'movie_providers':
        tmdb.show_movie_providers()
        ...
        if sources:
            scrapers.play_source(sources, title, year, fanart, poster, plot)
        else:
