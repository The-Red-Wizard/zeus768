"""
Tinklepad Debrid Manager v1.6
Handles Real-Debrid, AllDebrid, Premiumize and Torbox authorization and link resolution
Fixed:
  - Token persistence in Kodi 21 (Omega)
  - Torbox: bypass_cache on mylist polls (without this, Torbox returns stale
    cached status for up to 600s and resolves never finish)
  - Torbox: checkcached pre-flight using extracted magnet hash so uncached
    torrents fail fast instead of looping
  - Torbox: accept extended ready states (cached / completed / uploading)
  - Torbox: handle DUPLICATE_ITEM responses (re-adding an existing magnet)
"""
import xbmc
import xbmcgui
import xbmcaddon
import xbmcvfs
import requests
import time
import os
import json
import re
from urllib.parse import unquote

# API Endpoints
RD_BASE = 'https://api.real-debrid.com/rest/1.0'
RD_OAUTH = 'https://api.real-debrid.com/oauth/v2'
AD_BASE = 'https://api.alldebrid.com/v4'
PM_BASE = 'https://www.premiumize.me/api'
PM_TOKEN_URL = 'https://www.premiumize.me/token'
TB_BASE = 'https://api.torbox.app/v1/api'

# Real-Debrid Client ID (public)
RD_CLIENT_ID = 'X245A4XAIBGVM'

# Premiumize Client ID (public, used by ResolveURL community)
PM_CLIENT_ID = '522962560'

# Addon ID
ADDON_ID = 'plugin.video.tinklepad'


def get_addon_data_path():
    """Get the addon userdata path"""
    try:
        addon = xbmcaddon.Addon(ADDON_ID)
        return xbmcvfs.translatePath(addon.getAddonInfo('profile'))
    except:
        return xbmcvfs.translatePath('special://userdata/addon_data/plugin.video.tinklepad/')


def get_token_file_path():
    """Get path to token storage file"""
    data_path = get_addon_data_path()
    if not xbmcvfs.exists(data_path):
        xbmcvfs.mkdirs(data_path)
    return os.path.join(data_path, 'debrid_tokens.json')


class DebridManager:
    def __init__(self):
        self._token_cache = None
        self._cache_time = 0
        self._load_tokens()
    
    def _load_tokens(self):
        """Load tokens from file"""
        try:
            token_file = get_token_file_path()
            if xbmcvfs.exists(token_file):
                f = xbmcvfs.File(token_file, 'r')
                data = f.read()
                f.close()
                if data:
                    self._token_cache = json.loads(data)
                    xbmc.log(f'[Tinklepad] Loaded tokens from file: {list(self._token_cache.keys())}', xbmc.LOGINFO)
                    return
        except Exception as e:
            xbmc.log(f'[Tinklepad] Failed to load tokens from file: {e}', xbmc.LOGERROR)
        
        self._token_cache = {}
    
    def _save_tokens(self):
        """Save tokens to file"""
        try:
            token_file = get_token_file_path()
            data_path = get_addon_data_path()
            
            # Ensure directory exists
            if not xbmcvfs.exists(data_path):
                xbmcvfs.mkdirs(data_path)
            
            f = xbmcvfs.File(token_file, 'w')
            f.write(json.dumps(self._token_cache, indent=2))
            f.close()
            xbmc.log(f'[Tinklepad] Saved tokens to file', xbmc.LOGINFO)
            return True
        except Exception as e:
            xbmc.log(f'[Tinklepad] Failed to save tokens: {e}', xbmc.LOGERROR)
            return False
    
    def get_token(self, key):
        """Get a token value"""
        if self._token_cache is None:
            self._load_tokens()
        
        value = self._token_cache.get(key, '')
        xbmc.log(f'[Tinklepad] Get {key} = {value[:20] if value and len(str(value)) > 20 else value}', xbmc.LOGDEBUG)
        return value
    
    def set_token(self, key, value):
        """Set a token value and save to file"""
        if self._token_cache is None:
            self._token_cache = {}
        
        self._token_cache[key] = str(value)
        xbmc.log(f'[Tinklepad] Set {key} = {value[:20] if len(str(value)) > 20 else value}...', xbmc.LOGINFO)
        
        # Also try to save to Kodi settings for UI display
        try:
            addon = xbmcaddon.Addon(ADDON_ID)
            addon.setSetting(key, str(value))
        except:
            pass
        
        return self._save_tokens()
    
    def clear_tokens(self, prefix):
        """Clear all tokens with given prefix"""
        if self._token_cache is None:
            self._token_cache = {}
        
        keys_to_remove = [k for k in self._token_cache if k.startswith(prefix)]
        for key in keys_to_remove:
            del self._token_cache[key]
        
        self._save_tokens()
    
    # ==================== REAL-DEBRID ====================
    def rd_enabled(self):
        """Check if Real-Debrid has a valid token"""
        token = self.get_token('rd.token')
        return bool(token and len(token) > 10)
    
    def rd_authorized(self):
        """Check if we have a valid Real-Debrid token"""
        token = self.get_token('rd.token')
        is_auth = bool(token and len(token) > 10)
        xbmc.log(f'[Tinklepad] RD authorized check: {is_auth} (token len: {len(token) if token else 0})', xbmc.LOGINFO)
        return is_auth
    
    def rd_authorize(self):
        """OAuth device code flow for Real-Debrid"""
        try:
            xbmc.log('[Tinklepad] RD: Starting authorization...', xbmc.LOGINFO)
            
            # Step 1: Get device code
            response = requests.get(
                f'{RD_OAUTH}/device/code',
                params={'client_id': RD_CLIENT_ID, 'new_credentials': 'yes'},
                timeout=15
            )
            data = response.json()
            
            xbmc.log(f'[Tinklepad] RD: Got device code response: {list(data.keys())}', xbmc.LOGINFO)
            
            device_code = data['device_code']
            user_code = data['user_code']
            verification_url = data['verification_url']
            interval = data.get('interval', 5)
            expires_in = data.get('expires_in', 600)
            
            # Show user the code
            dialog = xbmcgui.DialogProgress()
            dialog.create(
                'Real-Debrid Authorization',
                f'Go to: [COLOR gold]{verification_url}[/COLOR]\n\nEnter code: [COLOR gold]{user_code}[/COLOR]\n\nWaiting for authorization...'
            )
            
            # Step 2: Poll for authorization
            start_time = time.time()
            while time.time() - start_time < expires_in:
                if dialog.iscanceled():
                    dialog.close()
                    return False
                
                xbmc.sleep(interval * 1000)
                
                try:
                    cred_response = requests.get(
                        f'{RD_OAUTH}/device/credentials',
                        params={'client_id': RD_CLIENT_ID, 'code': device_code},
                        timeout=15
                    )
                    cred_data = cred_response.json()
                    
                    xbmc.log(f'[Tinklepad] RD: Credentials response: {cred_data}', xbmc.LOGDEBUG)
                    
                    if 'client_id' in cred_data and 'client_secret' in cred_data:
                        # Got credentials, now get token
                        client_id = cred_data['client_id']
                        client_secret = cred_data['client_secret']
                        
                        token_response = requests.post(
                            f'{RD_OAUTH}/token',
                            data={
                                'client_id': client_id,
                                'client_secret': client_secret,
                                'code': device_code,
                                'grant_type': 'http://oauth.net/grant_type/device/1.0'
                            },
                            timeout=15
                        )
                        token_data = token_response.json()
                        
                        xbmc.log(f'[Tinklepad] RD: Token response keys: {list(token_data.keys())}', xbmc.LOGINFO)
                        
                        if 'access_token' in token_data:
                            access_token = token_data['access_token']
                            refresh_token = token_data.get('refresh_token', '')
                            expires_in_token = token_data.get('expires_in', 0)
                            expiry_time = str(int(time.time()) + expires_in_token)
                            
                            xbmc.log('[Tinklepad] RD: Saving credentials to file...', xbmc.LOGINFO)
                            
                            # Save all credentials to file
                            self.set_token('rd.token', access_token)
                            self.set_token('rd.refresh', refresh_token)
                            self.set_token('rd.expiry', expiry_time)
                            self.set_token('rd.client_id', client_id)
                            self.set_token('rd.secret', client_secret)
                            
                            # Get username
                            username = self._rd_get_username(access_token)
                            if username:
                                self.set_token('rd.username', username)
                            
                            dialog.close()
                            
                            # Verify save worked
                            saved_token = self.get_token('rd.token')
                            xbmc.log(f'[Tinklepad] RD: Verification - saved token length: {len(saved_token) if saved_token else 0}', xbmc.LOGINFO)
                            
                            xbmcgui.Dialog().notification('Tinklepad', 'Real-Debrid Authorized!', xbmcgui.NOTIFICATION_INFO, 3000)
                            xbmc.log('[Tinklepad] RD: Authorization successful!', xbmc.LOGINFO)
                            return True
                            
                except Exception as e:
                    xbmc.log(f'[Tinklepad] RD poll error: {e}', xbmc.LOGDEBUG)
                    continue
            
            dialog.close()
            xbmcgui.Dialog().notification('Tinklepad', 'Authorization timed out', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False
            
        except Exception as e:
            xbmc.log(f'[Tinklepad] RD auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False
    
    def _rd_get_username(self, token=None):
        """Fetch RD username"""
        try:
            if not token:
                token = self.get_token('rd.token')
            if not token:
                return None
                
            response = requests.get(
                f'{RD_BASE}/user',
                headers={'Authorization': f'Bearer {token}'},
                timeout=10
            )
            data = response.json()
            xbmc.log(f'[Tinklepad] RD User data: {data}', xbmc.LOGINFO)
            
            username = data.get('username', '') or data.get('email', '').split('@')[0]
            return username
        except Exception as e:
            xbmc.log(f'[Tinklepad] RD: Failed to get username: {e}', xbmc.LOGERROR)
            return None
    
    def rd_refresh_token(self):
        """Refresh Real-Debrid token"""
        try:
            refresh = self.get_token('rd.refresh')
            client_id = self.get_token('rd.client_id')
            client_secret = self.get_token('rd.secret')
            
            if not all([refresh, client_id, client_secret]):
                xbmc.log('[Tinklepad] RD: Missing refresh credentials', xbmc.LOGWARNING)
                return False
            
            response = requests.post(
                f'{RD_OAUTH}/token',
                data={
                    'client_id': client_id,
                    'client_secret': client_secret,
                    'refresh_token': refresh,
                    'grant_type': 'refresh_token'
                },
                timeout=15
            )
            data = response.json()
            
            if 'access_token' in data:
                self.set_token('rd.token', data['access_token'])
                self.set_token('rd.refresh', data.get('refresh_token', refresh))
                self.set_token('rd.expiry', str(int(time.time()) + data.get('expires_in', 0)))
                xbmc.log('[Tinklepad] RD: Token refreshed', xbmc.LOGINFO)
                return True
        except Exception as e:
            xbmc.log(f'[Tinklepad] RD refresh error: {e}', xbmc.LOGERROR)
        return False
    
    def rd_resolve(self, url):
        """Resolve a link through Real-Debrid"""
        try:
            token = self.get_token('rd.token')
            
            if not token:
                xbmc.log('[Tinklepad] RD: No token available', xbmc.LOGWARNING)
                return None
            
            # Check if token needs refresh
            expiry = self.get_token('rd.expiry')
            if expiry:
                try:
                    if int(expiry) < time.time():
                        xbmc.log('[Tinklepad] RD: Token expired, refreshing...', xbmc.LOGINFO)
                        if not self.rd_refresh_token():
                            return None
                        token = self.get_token('rd.token')
                except:
                    pass
            
            headers = {'Authorization': f'Bearer {token}'}
            
            # Handle magnets differently
            if url.startswith('magnet:'):
                # Add magnet to RD
                add_response = requests.post(
                    f'{RD_BASE}/torrents/addMagnet',
                    headers=headers,
                    data={'magnet': url},
                    timeout=15
                )
                add_data = add_response.json()
                
                if 'id' in add_data:
                    torrent_id = add_data['id']
                    
                    # Select all files
                    requests.post(
                        f'{RD_BASE}/torrents/selectFiles/{torrent_id}',
                        headers=headers,
                        data={'files': 'all'},
                        timeout=15
                    )
                    
                    # Wait for it to be ready
                    for _ in range(30):
                        info_response = requests.get(
                            f'{RD_BASE}/torrents/info/{torrent_id}',
                            headers=headers,
                            timeout=15
                        )
                        info_data = info_response.json()
                        
                        if info_data.get('status') == 'downloaded':
                            links = info_data.get('links', [])
                            if links:
                                # Unrestrict the first link
                                unrestrict_response = requests.post(
                                    f'{RD_BASE}/unrestrict/link',
                                    headers=headers,
                                    data={'link': links[0]},
                                    timeout=15
                                )
                                unrestrict_data = unrestrict_response.json()
                                
                                if 'download' in unrestrict_data:
                                    return unrestrict_data['download']
                            break
                        elif info_data.get('status') in ['magnet_error', 'error', 'dead']:
                            xbmc.log(f'[Tinklepad] RD: Torrent error: {info_data.get("status")}', xbmc.LOGWARNING)
                            break
                        
                        xbmc.sleep(2000)
            else:
                # Regular link - unrestrict
                response = requests.post(
                    f'{RD_BASE}/unrestrict/link',
                    headers=headers,
                    data={'link': url},
                    timeout=15
                )
                data = response.json()
                
                if 'download' in data:
                    xbmc.log(f'[Tinklepad] RD resolved successfully', xbmc.LOGINFO)
                    return data['download']
                elif 'error' in data:
                    error_code = data.get('error_code', 0)
                    error_msg = data.get('error', 'Unknown error')
                    xbmc.log(f'[Tinklepad] RD error: {error_msg} (code: {error_code})', xbmc.LOGWARNING)
                    
                    # Check for hoster_unavailable or similar errors that might work with remote upload
                    # Error codes: hoster_unavailable, hoster_not_free, hoster_limit_reached
                    if error_code in [23, 24, 25] or 'hoster_unavailable' in error_msg.lower() or 'unavailable' in error_msg.lower():
                        xbmc.log(f'[Tinklepad] RD: Direct unrestrict failed, trying remote download fallback...', xbmc.LOGINFO)
                        return self._rd_remote_download_fallback(url, headers)
                    
        except Exception as e:
            xbmc.log(f'[Tinklepad] RD resolve error: {e}', xbmc.LOGERROR)
        return None
    
    def _rd_remote_download_fallback(self, url, headers):
        """
        Fallback: Use Real-Debrid's remote download (downloads) feature
        when direct unrestriction fails (e.g., hoster_unavailable for Nitroflare/Rapidgator)
        """
        try:
            xbmc.log(f'[Tinklepad] RD: Adding link to remote downloads: {url[:50]}...', xbmc.LOGINFO)
            
            # Add link to RD downloads (remote upload)
            add_response = requests.post(
                f'{RD_BASE}/downloads/add',
                headers=headers,
                data={'link': url},
                timeout=30
            )
            add_data = add_response.json()
            
            xbmc.log(f'[Tinklepad] RD: Remote download add response: {add_data}', xbmc.LOGDEBUG)
            
            if 'id' in add_data:
                download_id = add_data['id']
                xbmc.log(f'[Tinklepad] RD: Remote download added with ID: {download_id}', xbmc.LOGINFO)
                
                # Show progress dialog
                dialog = xbmcgui.DialogProgress()
                dialog.create('Real-Debrid', 'Downloading file to Real-Debrid cloud...\nThis may take a few minutes.')
                
                # Poll for completion (max 5 minutes)
                max_wait = 300
                start_time = time.time()
                
                while time.time() - start_time < max_wait:
                    if dialog.iscanceled():
                        xbmc.log('[Tinklepad] RD: Remote download cancelled by user', xbmc.LOGINFO)
                        dialog.close()
                        return None
                    
                    # Check download status
                    status_response = requests.get(
                        f'{RD_BASE}/downloads/info/{download_id}',
                        headers=headers,
                        timeout=15
                    )
                    status_data = status_response.json()
                    
                    status = status_data.get('status', '')
                    progress = status_data.get('progress', 0)
                    
                    xbmc.log(f'[Tinklepad] RD: Remote download status: {status}, progress: {progress}%', xbmc.LOGDEBUG)
                    
                    # Update dialog
                    elapsed = int(time.time() - start_time)
                    dialog.update(int(progress), f'Status: {status}\nProgress: {progress}%\nElapsed: {elapsed}s')
                    
                    if status == 'downloaded':
                        # Download complete - get the link
                        download_link = status_data.get('download', '')
                        if download_link:
                            dialog.close()
                            xbmc.log(f'[Tinklepad] RD: Remote download complete!', xbmc.LOGINFO)
                            return download_link
                        else:
                            # Try to get link from the download info
                            link = status_data.get('link', '')
                            if link:
                                # Unrestrict this RD link
                                unrestrict_response = requests.post(
                                    f'{RD_BASE}/unrestrict/link',
                                    headers=headers,
                                    data={'link': link},
                                    timeout=15
                                )
                                unrestrict_data = unrestrict_response.json()
                                if 'download' in unrestrict_data:
                                    dialog.close()
                                    return unrestrict_data['download']
                        dialog.close()
                        break
                    
                    elif status in ['error', 'dead', 'virus', 'magnet_error']:
                        xbmc.log(f'[Tinklepad] RD: Remote download failed with status: {status}', xbmc.LOGWARNING)
                        dialog.close()
                        return None
                    
                    elif status == 'queued' or status == 'downloading':
                        # Still in progress
                        pass
                    
                    xbmc.sleep(3000)  # Poll every 3 seconds
                
                dialog.close()
                xbmc.log('[Tinklepad] RD: Remote download timed out', xbmc.LOGWARNING)
                
            elif 'error' in add_data:
                xbmc.log(f'[Tinklepad] RD: Failed to add remote download: {add_data["error"]}', xbmc.LOGWARNING)
                
        except Exception as e:
            xbmc.log(f'[Tinklepad] RD: Remote download fallback error: {e}', xbmc.LOGERROR)
        
        return None
    
    # ==================== ALLDEBRID ====================
    def ad_enabled(self):
        """Check if AllDebrid has a valid token"""
        token = self.get_token('ad.token')
        return bool(token and len(token) > 10)
    
    def ad_authorized(self):
        token = self.get_token('ad.token')
        is_auth = bool(token and len(token) > 10)
        xbmc.log(f'[Tinklepad] AD authorized check: {is_auth}', xbmc.LOGINFO)
        return is_auth
    
    def ad_authorize(self):
        """PIN-based authorization for AllDebrid"""
        try:
            xbmc.log('[Tinklepad] AD: Starting authorization...', xbmc.LOGINFO)
            
            # Step 1: Get PIN
            response = requests.get(
                f'{AD_BASE}/pin/get',
                params={'agent': 'Tinklepad'},
                timeout=15
            )
            data = response.json()
            
            if data.get('status') != 'success':
                raise Exception(data.get('error', {}).get('message', 'Unknown error'))
            
            pin_data = data['data']
            pin = pin_data['pin']
            check_url = pin_data['check_url']
            user_url = pin_data['user_url']
            expires_in = pin_data.get('expires_in', 600)
            
            # Show user the PIN
            dialog = xbmcgui.DialogProgress()
            dialog.create(
                'AllDebrid Authorization',
                f'Go to: [COLOR gold]{user_url}[/COLOR]\n\nEnter PIN: [COLOR gold]{pin}[/COLOR]\n\nWaiting for authorization...'
            )
            
            # Step 2: Poll for authorization
            start_time = time.time()
            while time.time() - start_time < expires_in:
                if dialog.iscanceled():
                    dialog.close()
                    return False
                
                xbmc.sleep(5000)
                
                try:
                    check_response = requests.get(check_url, params={'agent': 'Tinklepad'}, timeout=15)
                    check_data = check_response.json()
                    
                    if check_data.get('status') == 'success' and check_data.get('data', {}).get('activated'):
                        apikey = check_data['data']['apikey']
                        
                        # Save token to file
                        self.set_token('ad.token', apikey)
                        
                        # Get username
                        username = self._ad_get_username(apikey)
                        if username:
                            self.set_token('ad.username', username)
                        
                        dialog.close()
                        xbmcgui.Dialog().notification('Tinklepad', 'AllDebrid Authorized!', xbmcgui.NOTIFICATION_INFO, 3000)
                        xbmc.log('[Tinklepad] AD: Authorization successful!', xbmc.LOGINFO)
                        return True
                        
                except Exception as e:
                    xbmc.log(f'[Tinklepad] AD poll error: {e}', xbmc.LOGDEBUG)
                    continue
            
            dialog.close()
            xbmcgui.Dialog().notification('Tinklepad', 'Authorization timed out', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False
            
        except Exception as e:
            xbmc.log(f'[Tinklepad] AD auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False
    
    def _ad_get_username(self, token=None):
        """Fetch AD username"""
        try:
            if not token:
                token = self.get_token('ad.token')
            response = requests.get(
                f'{AD_BASE}/user',
                params={'agent': 'Tinklepad', 'apikey': token},
                timeout=10
            )
            data = response.json()
            if data.get('status') == 'success':
                return data['data'].get('username', 'Unknown')
        except Exception as e:
            xbmc.log(f'[Tinklepad] AD: Failed to get username: {e}', xbmc.LOGERROR)
        return None
    
    def ad_resolve(self, url):
        """Resolve a link through AllDebrid"""
        try:
            token = self.get_token('ad.token')
            if not token:
                return None
            
            # Handle magnets
            if url.startswith('magnet:'):
                # Upload magnet
                upload_response = requests.get(
                    f'{AD_BASE}/magnet/upload',
                    params={'agent': 'Tinklepad', 'apikey': token, 'magnets[]': url},
                    timeout=15
                )
                upload_data = upload_response.json()
                
                if upload_data.get('status') == 'success':
                    magnets = upload_data.get('data', {}).get('magnets', [])
                    if magnets:
                        magnet_id = magnets[0].get('id')
                        
                        # Wait for ready
                        for _ in range(30):
                            status_response = requests.get(
                                f'{AD_BASE}/magnet/status',
                                params={'agent': 'Tinklepad', 'apikey': token, 'id': magnet_id},
                                timeout=15
                            )
                            status_data = status_response.json()
                            
                            if status_data.get('status') == 'success':
                                magnet_info = status_data.get('data', {}).get('magnets', {})
                                if magnet_info.get('status') == 'Ready':
                                    links = magnet_info.get('links', [])
                                    if links:
                                        return self.ad_resolve(links[0]['link'])
                                elif magnet_info.get('status') in ['Error']:
                                    break
                            
                            xbmc.sleep(2000)
            else:
                response = requests.get(
                    f'{AD_BASE}/link/unlock',
                    params={'agent': 'Tinklepad', 'apikey': token, 'link': url},
                    timeout=15
                )
                data = response.json()
                
                if data.get('status') == 'success' and 'link' in data.get('data', {}):
                    resolved = data['data']['link']
                    xbmc.log(f'[Tinklepad] AD resolved successfully', xbmc.LOGINFO)
                    return resolved
                elif 'error' in data:
                    xbmc.log(f'[Tinklepad] AD error: {data.get("error")}', xbmc.LOGWARNING)
                    
        except Exception as e:
            xbmc.log(f'[Tinklepad] AD resolve error: {e}', xbmc.LOGERROR)
        return None
    
    # ==================== PREMIUMIZE ====================
    def pm_enabled(self):
        """Check if Premiumize has a valid token"""
        token = self.get_token('pm.token')
        return bool(token and len(token) > 10)
    
    def pm_authorized(self):
        token = self.get_token('pm.token')
        is_auth = bool(token and len(token) > 10)
        xbmc.log(f'[Tinklepad] PM authorized check: {is_auth}', xbmc.LOGINFO)
        return is_auth
    
    def pm_authorize(self):
        """Premiumize authorization. Lets the user choose between device pairing
        (recommended) and entering an API key manually."""
        try:
            xbmc.log('[Tinklepad] PM: Starting authorization...', xbmc.LOGINFO)

            choice = xbmcgui.Dialog().select(
                'Premiumize Authorization',
                [
                    'Pair this device (recommended)',
                    'Enter API key manually',
                ]
            )

            if choice == -1:
                return False
            if choice == 0:
                return self._pm_device_authorize()
            return self._pm_apikey_authorize()

        except Exception as e:
            xbmc.log(f'[Tinklepad] PM auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False

    def _pm_device_authorize(self):
        """OAuth device-code flow for Premiumize ('Get Code' pairing)."""
        try:
            xbmc.log('[Tinklepad] PM: Requesting device code...', xbmc.LOGINFO)

            # Step 1: Request device code
            response = requests.post(
                PM_TOKEN_URL,
                data={'response_type': 'device_code', 'client_id': PM_CLIENT_ID},
                timeout=15
            )
            data = response.json()
            xbmc.log(f'[Tinklepad] PM: device code keys: {list(data.keys())}', xbmc.LOGINFO)

            if 'device_code' not in data or 'user_code' not in data:
                msg = data.get('error', data.get('message', 'No code received from Premiumize'))
                xbmcgui.Dialog().ok('Tinklepad', f'Premiumize pairing failed:\n{msg}')
                return False

            device_code = data['device_code']
            user_code = data['user_code']
            verification_uri = data.get('verification_uri') or 'https://www.premiumize.me/device'
            interval = int(data.get('interval', 5)) or 5
            expires_in = int(data.get('expires_in', 600)) or 600

            # Step 2: Show user the code
            dialog = xbmcgui.DialogProgress()
            dialog.create(
                'Premiumize Authorization',
                f'Go to: [COLOR gold]{verification_uri}[/COLOR]\n\n'
                f'Enter code: [B][COLOR gold]{user_code}[/COLOR][/B]\n\n'
                f'Waiting for authorization...'
            )

            # Step 3: Poll for token
            start_time = time.time()
            while time.time() - start_time < expires_in:
                if dialog.iscanceled():
                    dialog.close()
                    return False

                xbmc.sleep(interval * 1000)

                try:
                    poll = requests.post(
                        PM_TOKEN_URL,
                        data={
                            'grant_type': 'device_code',
                            'client_id': PM_CLIENT_ID,
                            'code': device_code,
                        },
                        timeout=15
                    )
                    poll_data = poll.json()
                except Exception as e:
                    xbmc.log(f'[Tinklepad] PM poll error: {e}', xbmc.LOGDEBUG)
                    continue

                if 'access_token' in poll_data:
                    access_token = poll_data['access_token']
                    self.set_token('pm.token', access_token)

                    # Fetch username/customer_id
                    customer_id = self._pm_get_username(access_token) or 'Authorized'
                    self.set_token('pm.username', customer_id)

                    dialog.close()
                    xbmcgui.Dialog().notification(
                        'Tinklepad',
                        f'Premiumize Authorized! (ID: {customer_id})',
                        xbmcgui.NOTIFICATION_INFO, 3000
                    )
                    xbmc.log(f'[Tinklepad] PM: Authorization successful! Customer: {customer_id}', xbmc.LOGINFO)
                    return True

                err = poll_data.get('error', '')
                if err and err not in ('authorization_pending', 'slow_down'):
                    dialog.close()
                    xbmcgui.Dialog().ok('Tinklepad', f'Premiumize pairing failed:\n{err}')
                    return False

            dialog.close()
            xbmcgui.Dialog().notification('Tinklepad', 'Premiumize pairing timed out', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False

        except Exception as e:
            xbmc.log(f'[Tinklepad] PM device auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False

    def _pm_apikey_authorize(self):
        """Manual API key authorization for Premiumize."""
        try:
            dialog = xbmcgui.Dialog()
            api_key = dialog.input(
                'Premiumize API Key',
                '',
                xbmcgui.INPUT_ALPHANUM
            )
            api_key = (api_key or '').strip()

            if not api_key or len(api_key) < 5:
                dialog.notification('Tinklepad', 'No API key entered', xbmcgui.NOTIFICATION_WARNING, 3000)
                return False

            response = requests.get(
                f'{PM_BASE}/account/info',
                params={'apikey': api_key},
                timeout=10
            )
            data = response.json()

            if data.get('status') == 'success':
                self.set_token('pm.token', api_key)
                customer_id = str(data.get('customer_id', 'Unknown'))
                self.set_token('pm.username', customer_id)
                xbmcgui.Dialog().notification('Tinklepad', f'Premiumize Authorized! (ID: {customer_id})', xbmcgui.NOTIFICATION_INFO, 3000)
                return True

            msg = data.get('message', 'Invalid API key')
            xbmcgui.Dialog().ok('Tinklepad', f'Premiumize authorization failed:\n{msg}\n\nGet your API key from premiumize.me/account')
            return False
        except Exception as e:
            xbmc.log(f'[Tinklepad] PM apikey auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False
    
    def _pm_get_username(self, token=None):
        """Fetch PM username. Tries Bearer first (OAuth tokens), falls back to apikey (legacy)."""
        try:
            if not token:
                token = self.get_token('pm.token')
            if not token:
                return None
            # Bearer header (works for OAuth access_token)
            try:
                r = requests.get(
                    f'{PM_BASE}/account/info',
                    headers={'Authorization': f'Bearer {token}'},
                    timeout=10
                )
                data = r.json()
                if data.get('status') == 'success':
                    return str(data.get('customer_id', 'Unknown'))
            except Exception as e:
                xbmc.log(f'[Tinklepad] PM bearer username err: {e}', xbmc.LOGDEBUG)
            # Fallback to apikey (legacy API keys)
            r = requests.get(
                f'{PM_BASE}/account/info',
                params={'apikey': token},
                timeout=10
            )
            data = r.json()
            if data.get('status') == 'success':
                return str(data.get('customer_id', 'Unknown'))
        except Exception as e:
            xbmc.log(f'[Tinklepad] PM: Failed to get username: {e}', xbmc.LOGERROR)
        return None
    
    def pm_resolve(self, url):
        """Resolve a link through Premiumize. Tries Bearer first, then apikey."""
        try:
            token = self.get_token('pm.token')
            if not token:
                return None

            data = None
            # Try Bearer (OAuth)
            try:
                r = requests.post(
                    f'{PM_BASE}/transfer/directdl',
                    data={'src': url},
                    headers={'Authorization': f'Bearer {token}'},
                    timeout=30
                )
                data = r.json()
            except Exception as e:
                xbmc.log(f'[Tinklepad] PM bearer resolve err: {e}', xbmc.LOGDEBUG)
                data = None

            # Fallback to apikey for legacy keys
            if not data or data.get('status') != 'success':
                r = requests.post(
                    f'{PM_BASE}/transfer/directdl',
                    data={'apikey': token, 'src': url},
                    timeout=30
                )
                data = r.json()

            if data.get('status') == 'success':
                content = data.get('content', [])
                if content:
                    best = max(content, key=lambda x: x.get('size', 0))
                    resolved = best.get('link')
                    if resolved:
                        xbmc.log(f'[Tinklepad] PM resolved successfully', xbmc.LOGINFO)
                        return resolved
        except Exception as e:
            xbmc.log(f'[Tinklepad] PM resolve error: {e}', xbmc.LOGERROR)
        return None
    
    # ==================== TORBOX ====================
    # Video extensions used to pick playable files
    TB_VIDEO_EXTS = ('.mkv', '.mp4', '.avi', '.m4v', '.mov', '.wmv', '.flv', '.ts', '.webm', '.mpg', '.mpeg')

    def tb_enabled(self):
        """Check if Torbox has a valid token"""
        token = self.get_token('tb.token')
        return bool(token and len(token) > 10)

    def tb_authorized(self):
        token = self.get_token('tb.token')
        is_auth = bool(token and len(token) > 10)
        xbmc.log(f'[Tinklepad] TB authorized check: {is_auth}', xbmc.LOGINFO)
        return is_auth

    def tb_authorize(self):
        """API key based authorization for Torbox.

        Uses positional arguments to xbmcgui.Dialog().input for maximum
        compatibility across Kodi 19/20/21 (the ``defaultt`` kwarg has been
        flaky on some skins/forks and caused silent failures).
        """
        try:
            xbmc.log('[Tinklepad] TB: Starting authorization...', xbmc.LOGINFO)

            dialog = xbmcgui.Dialog()
            # Positional args: heading, defaultt, type
            # type=xbmcgui.INPUT_ALPHANUM accepts letters/digits AND symbols
            # (the on-screen keyboard exposes hyphens, dots, etc. via shift)
            api_key = dialog.input(
                'Enter Torbox API Key (from torbox.app/settings)',
                '',
                xbmcgui.INPUT_ALPHANUM
            )

            # Normalize: strip whitespace that often sneaks in via copy/paste
            api_key = (api_key or '').strip()

            if not api_key:
                dialog.notification('Tinklepad', 'No API key entered', xbmcgui.NOTIFICATION_WARNING, 3000)
                return False

            if len(api_key) < 10:
                dialog.ok('Tinklepad', 'The API key looks too short.\n\nGet your key from:\nhttps://torbox.app/settings')
                return False

            xbmc.log(f'[Tinklepad] TB: Verifying API key (len={len(api_key)})...', xbmc.LOGINFO)

            # Verify API key against /user/me
            try:
                response = requests.get(
                    f'{TB_BASE}/user/me',
                    headers={'Authorization': f'Bearer {api_key}'},
                    timeout=20
                )
            except requests.exceptions.RequestException as e:
                xbmc.log(f'[Tinklepad] TB: Network error during auth: {e}', xbmc.LOGERROR)
                dialog.ok('Tinklepad', f'Network error contacting Torbox:\n{e}\n\nCheck your internet connection and try again.')
                return False

            # Handle non-2xx separately for clearer messaging
            if response.status_code == 401 or response.status_code == 403:
                dialog.ok('Tinklepad', 'Torbox rejected the API key (401/403).\n\nMake sure you copied the FULL key from torbox.app/settings (no spaces).')
                return False

            try:
                data = response.json()
            except ValueError:
                xbmc.log(f'[Tinklepad] TB: Non-JSON response (status {response.status_code}): {response.text[:200]}', xbmc.LOGERROR)
                dialog.ok('Tinklepad', f'Unexpected Torbox response (HTTP {response.status_code}).\n\nPlease try again in a moment.')
                return False

            if data.get('success') and data.get('data'):
                self.set_token('tb.token', api_key)
                user_data = data.get('data', {})
                username = (
                    user_data.get('email') or
                    user_data.get('customer') or
                    str(user_data.get('id', 'Authorized'))
                )
                self.set_token('tb.username', username)
                xbmcgui.Dialog().notification('Tinklepad', f'Torbox Authorized! ({username})', xbmcgui.NOTIFICATION_INFO, 3000)
                xbmc.log(f'[Tinklepad] TB: Authorization successful! User: {username}', xbmc.LOGINFO)
                return True

            msg = data.get('detail') or data.get('error') or 'Invalid API key'
            xbmc.log(f'[Tinklepad] TB: Auth failed: {msg} | full: {data}', xbmc.LOGWARNING)
            dialog.ok('Tinklepad', f'Torbox authorization failed:\n{msg}\n\nGet your API key from torbox.app/settings')
            return False
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB auth error: {e}', xbmc.LOGERROR)
            xbmcgui.Dialog().notification('Tinklepad', f'Auth failed: {e}', xbmcgui.NOTIFICATION_ERROR, 3000)
            return False

    def _tb_get_username(self, token=None):
        """Fetch Torbox username/email"""
        try:
            if not token:
                token = self.get_token('tb.token')
            if not token:
                return None
            response = requests.get(
                f'{TB_BASE}/user/me',
                headers={'Authorization': f'Bearer {token}'},
                timeout=10
            )
            data = response.json()
            if data.get('success') and data.get('data'):
                user_data = data['data']
                return user_data.get('email') or user_data.get('customer') or str(user_data.get('id', 'Authorized'))
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB: Failed to get username: {e}', xbmc.LOGERROR)
        return None

    def _tb_pick_video_file(self, files):
        """Choose the largest playable video file from a Torbox files list."""
        if not files:
            return None
        videos = [
            f for f in files
            if str(f.get('short_name') or f.get('name') or '').lower().endswith(self.TB_VIDEO_EXTS)
        ]
        if not videos:
            return None
        videos.sort(key=lambda x: x.get('size', 0), reverse=True)
        return videos[0]

    @staticmethod
    def _tb_extract_hash(magnet_or_url):
        """Extract the BTIH info-hash (40-char hex or 32-char base32) from a magnet URI."""
        if not magnet_or_url:
            return None
        try:
            m = re.search(r'xt=urn:btih:([A-Za-z0-9]+)', unquote(magnet_or_url), re.IGNORECASE)
            if m:
                return m.group(1).lower()
        except Exception:
            pass
        return None

    def _tb_check_cached(self, info_hash, headers):
        """Return True if the given info-hash is instantly available on Torbox."""
        if not info_hash:
            return False
        try:
            r = requests.get(
                f'{TB_BASE}/torrents/checkcached',
                headers=headers,
                params={'hash': info_hash, 'format': 'object'},
                timeout=15,
            )
            data = r.json()
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB: checkcached error: {e}', xbmc.LOGDEBUG)
            return False

        if not data.get('success'):
            return False
        d = data.get('data') or {}
        # ``object`` format returns a dict keyed by hash; ``list`` returns a list.
        if isinstance(d, dict):
            entry = d.get(info_hash) or d.get(info_hash.upper()) or d.get(info_hash.lower())
            return bool(entry)
        if isinstance(d, list):
            return any(
                (x.get('hash') or '').lower() == info_hash.lower() for x in d
            )
        return False

    def tb_resolve(self, url):
        """Resolve a magnet, NZB or DDL link through Torbox.

        magnet:/.torrent → /torrents/createtorrent → /torrents/requestdl
        .nzb / Usenet    → /usenet/createusenetdownload → /usenet/requestdl
        anything else    → /webdl/createwebdownload → /webdl/requestdl
        """
        try:
            token = self.get_token('tb.token')
            if not token:
                return None

            lurl = url.lower()
            if url.startswith('magnet:') or lurl.endswith('.torrent'):
                return self._tb_resolve_torrent(url, token)
            if lurl.endswith('.nzb') or '/getnzb/' in lurl or 'nzb=' in lurl:
                resolved = self._tb_resolve_usenet(url, token)
                if resolved:
                    return resolved
                # Fall through to webdl if Usenet rejected (e.g. not an NZB after all)
            return self._tb_resolve_webdl(url, token)
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB resolve error: {e}', xbmc.LOGERROR)
            return None

    def _tb_resolve_torrent(self, url, token):
        headers = {'Authorization': f'Bearer {token}'}

        # Pre-flight: check cached availability. Uncached torrents on Torbox
        # can take hours to complete (it actually downloads them for you), so
        # for instant Kodi playback we only proceed when cached.
        info_hash = self._tb_extract_hash(url)
        if info_hash:
            is_cached = self._tb_check_cached(info_hash, headers)
            xbmc.log(f'[Tinklepad] TB: checkcached(hash={info_hash[:12]}...) -> {is_cached}', xbmc.LOGINFO)
            if not is_cached:
                # Not cached. Don't waste the user's time looping; let other
                # debrids try (or scraper falls back).
                xbmc.log('[Tinklepad] TB: torrent not cached, skipping', xbmc.LOGINFO)
                return None

        # Step 1: Add magnet/torrent. Torbox accepts multipart/form-data
        # most reliably (use ``files=`` to force multipart).
        try:
            add_response = requests.post(
                f'{TB_BASE}/torrents/createtorrent',
                headers=headers,
                files={
                    'magnet': (None, url),
                    'seed': (None, '1'),          # 1 = auto seed (Torbox default)
                    'allow_zip': (None, 'false'),
                },
                timeout=30
            )
        except requests.exceptions.RequestException as e:
            xbmc.log(f'[Tinklepad] TB: createtorrent network error: {e}', xbmc.LOGERROR)
            return None

        try:
            add_data = add_response.json()
        except ValueError:
            xbmc.log(f'[Tinklepad] TB: createtorrent non-JSON (HTTP {add_response.status_code}): {add_response.text[:200]}', xbmc.LOGERROR)
            return None

        xbmc.log(f'[Tinklepad] TB: createtorrent -> {add_data}', xbmc.LOGINFO)

        d = add_data.get('data') or {}
        torrent_id = None

        if add_data.get('success'):
            torrent_id = d.get('torrent_id') or d.get('id') or d.get('queued_id')
        else:
            # Common case: DUPLICATE_ITEM — the magnet is already in the
            # user's library. The API still puts the existing torrent_id in
            # ``data`` for this error, so try to recover it.
            err = (add_data.get('error') or '').upper()
            if err == 'DUPLICATE_ITEM':
                torrent_id = d.get('torrent_id') or d.get('id')
                xbmc.log(f'[Tinklepad] TB: duplicate item, reusing torrent_id={torrent_id}', xbmc.LOGINFO)
            else:
                detail = add_data.get('detail') or add_data.get('error') or 'unknown error'
                xbmc.log(f'[Tinklepad] TB: createtorrent failed: {detail}', xbmc.LOGWARNING)
                return None

        if not torrent_id:
            xbmc.log('[Tinklepad] TB: No torrent_id in createtorrent response', xbmc.LOGWARNING)
            return None

        # Step 2: Poll mylist until ready (cached or downloaded). We pass
        # ``bypass_cache=true`` because mylist is otherwise cached for ~600s
        # and would never report the freshly-added torrent as ready.
        files = self._tb_wait_torrent_ready(torrent_id, headers, max_iters=45, sleep_ms=2000)
        if not files:
            xbmc.log('[Tinklepad] TB: Torrent not ready / not cached (queued in cloud)', xbmc.LOGWARNING)
            return None

        # Step 3: Pick the best video file
        chosen = self._tb_pick_video_file(files)
        if not chosen:
            xbmc.log('[Tinklepad] TB: No playable video file in torrent', xbmc.LOGWARNING)
            return None
        file_id = chosen.get('id')

        # Step 4: Request download link
        return self._tb_request_dl(
            f'{TB_BASE}/torrents/requestdl',
            {'token': token, 'torrent_id': torrent_id, 'file_id': file_id},
            headers,
        )

    def _tb_wait_torrent_ready(self, torrent_id, headers, max_iters=45, sleep_ms=2000):
        """Poll /torrents/mylist?id=X until ready, return files list (or []).

        ``bypass_cache=true`` is critical — Torbox otherwise serves a 10-min
        stale cache and the freshly-added torrent never appears "finished".
        """
        ready_states = ('cached', 'completed', 'uploading', 'seeding')
        error_states = ('error', 'dead', 'failed', 'stalled (no seeds)', 'missingFiles')
        for i in range(max_iters):
            try:
                info_response = requests.get(
                    f'{TB_BASE}/torrents/mylist',
                    headers=headers,
                    params={'id': torrent_id, 'bypass_cache': 'true'},
                    timeout=15
                )
                info_data = info_response.json()
            except Exception as e:
                xbmc.log(f'[Tinklepad] TB: mylist poll error: {e}', xbmc.LOGDEBUG)
                xbmc.sleep(sleep_ms)
                continue

            if info_data.get('success') and info_data.get('data'):
                info = info_data['data']
                # If the API returned a list (legacy), grab the first match
                if isinstance(info, list):
                    info = next((x for x in info if x.get('id') == torrent_id), info[0] if info else {})
                state = (info.get('download_state') or '').lower()
                if (info.get('download_finished')
                        or info.get('download_present')
                        or info.get('cached')
                        or state in ready_states):
                    return info.get('files', []) or []
                if state in error_states:
                    xbmc.log(f'[Tinklepad] TB: Torrent error state: {state}', xbmc.LOGWARNING)
                    return []
                if i == 0 or i % 5 == 0:
                    xbmc.log(f'[Tinklepad] TB: Waiting (state={state}, progress={info.get("progress")})...', xbmc.LOGINFO)
            xbmc.sleep(sleep_ms)
        return []

    def _tb_resolve_webdl(self, url, token):
        """Resolve a direct download link via Torbox's web download service."""
        headers = {'Authorization': f'Bearer {token}'}
        xbmc.log(f'[Tinklepad] TB: Trying webdl for {url[:80]}...', xbmc.LOGINFO)

        try:
            add_response = requests.post(
                f'{TB_BASE}/webdl/createwebdownload',
                headers=headers,
                files={'link': (None, url)},
                timeout=30
            )
            add_data = add_response.json()
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB: webdl create error: {e}', xbmc.LOGERROR)
            return None

        xbmc.log(f'[Tinklepad] TB: webdl create -> {add_data}', xbmc.LOGINFO)

        if not add_data.get('success'):
            detail = add_data.get('detail') or add_data.get('error') or ''
            # Hoster not supported / not enabled
            xbmc.log(f'[Tinklepad] TB: webdl rejected: {detail}', xbmc.LOGWARNING)
            return None

        d = add_data.get('data') or {}
        webdl_id = d.get('webdownload_id') or d.get('id')
        if not webdl_id:
            return None

        # Poll for ready state
        files = self._tb_wait_webdl_ready(webdl_id, headers, max_iters=60, sleep_ms=2500)
        if not files:
            xbmc.log('[Tinklepad] TB: webdl not ready (still downloading)', xbmc.LOGWARNING)
            return None

        chosen = self._tb_pick_video_file(files)
        if not chosen:
            # Fall back to first file when nothing matches video exts (some DDLs)
            chosen = files[0]
        file_id = chosen.get('id')

        return self._tb_request_dl(
            f'{TB_BASE}/webdl/requestdl',
            {'token': token, 'web_id': webdl_id, 'file_id': file_id},
            headers,
        )

    def _tb_wait_webdl_ready(self, webdl_id, headers, max_iters=60, sleep_ms=2500):
        """Poll /webdl/mylist?id=X until the web download is ready."""
        for i in range(max_iters):
            try:
                info_response = requests.get(
                    f'{TB_BASE}/webdl/mylist',
                    headers=headers,
                    params={'id': webdl_id, 'bypass_cache': 'true'},
                    timeout=15
                )
                info_data = info_response.json()
            except Exception as e:
                xbmc.log(f'[Tinklepad] TB: webdl mylist poll error: {e}', xbmc.LOGDEBUG)
                xbmc.sleep(sleep_ms)
                continue

            if info_data.get('success') and info_data.get('data'):
                info = info_data['data']
                if isinstance(info, list):
                    info = next((x for x in info if x.get('id') == webdl_id), info[0] if info else {})
                state = (info.get('download_state') or '').lower()
                if (info.get('download_finished')
                        or info.get('download_present')
                        or state in ('cached', 'completed')):
                    return info.get('files', []) or []
                if state in ('error', 'dead', 'failed'):
                    xbmc.log(f'[Tinklepad] TB: webdl error state: {state}', xbmc.LOGWARNING)
                    return []
                if i == 0 or i % 5 == 0:
                    xbmc.log(f'[Tinklepad] TB: webdl waiting (state={state})...', xbmc.LOGINFO)
            xbmc.sleep(sleep_ms)
        return []

    def _tb_request_dl(self, endpoint, params, headers):
        """Hit a Torbox requestdl endpoint and return the playable URL."""
        try:
            dl_response = requests.get(endpoint, headers=headers, params=params, timeout=20)
            dl_data = dl_response.json()
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB: requestdl error: {e}', xbmc.LOGERROR)
            return None

        if dl_data.get('success') and dl_data.get('data'):
            resolved = dl_data['data']
            # data is a string URL
            if isinstance(resolved, str) and resolved.startswith('http'):
                xbmc.log('[Tinklepad] TB resolved successfully', xbmc.LOGINFO)
                return resolved
        xbmc.log(f'[Tinklepad] TB: requestdl failed: {dl_data.get("detail", dl_data)}', xbmc.LOGWARNING)
        return None

    def _tb_resolve_usenet(self, url, token):
        """Resolve an NZB link via Torbox's Usenet service."""
        headers = {'Authorization': f'Bearer {token}'}
        xbmc.log(f'[Tinklepad] TB: Trying usenet for {url[:80]}...', xbmc.LOGINFO)

        try:
            add_response = requests.post(
                f'{TB_BASE}/usenet/createusenetdownload',
                headers=headers,
                files={'link': (None, url)},
                timeout=30
            )
            add_data = add_response.json()
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB: usenet create error: {e}', xbmc.LOGERROR)
            return None

        xbmc.log(f'[Tinklepad] TB: usenet create -> {add_data}', xbmc.LOGINFO)

        if not add_data.get('success'):
            detail = add_data.get('detail') or add_data.get('error') or ''
            xbmc.log(f'[Tinklepad] TB: usenet rejected: {detail}', xbmc.LOGWARNING)
            return None

        d = add_data.get('data') or {}
        usenet_id = d.get('usenetdownload_id') or d.get('usenet_id') or d.get('id')
        if not usenet_id:
            xbmc.log('[Tinklepad] TB: No usenet id in response', xbmc.LOGWARNING)
            return None

        # Usenet downloads take a bit longer than cached webdl/torrents.
        # Poll for up to ~5 minutes.
        files = self._tb_wait_usenet_ready(usenet_id, headers, max_iters=100, sleep_ms=3000)
        if not files:
            xbmc.log('[Tinklepad] TB: usenet not ready (still downloading)', xbmc.LOGWARNING)
            return None

        chosen = self._tb_pick_video_file(files)
        if not chosen:
            chosen = files[0]
        file_id = chosen.get('id')

        return self._tb_request_dl(
            f'{TB_BASE}/usenet/requestdl',
            {'token': token, 'usenet_id': usenet_id, 'file_id': file_id},
            headers,
        )

    def _tb_wait_usenet_ready(self, usenet_id, headers, max_iters=100, sleep_ms=3000):
        """Poll /usenet/mylist until the NZB has finished downloading.

        ``mylist`` returns a list; we filter by ``id``. ``mylist?id=X`` is also
        supported and returns the single object directly. We try the targeted
        form first for efficiency.
        """
        for i in range(max_iters):
            try:
                info_response = requests.get(
                    f'{TB_BASE}/usenet/mylist',
                    headers=headers,
                    params={'id': usenet_id, 'bypass_cache': 'true'},
                    timeout=15
                )
                info_data = info_response.json()
            except Exception as e:
                xbmc.log(f'[Tinklepad] TB: usenet mylist poll error: {e}', xbmc.LOGDEBUG)
                xbmc.sleep(sleep_ms)
                continue

            if info_data.get('success') and info_data.get('data') is not None:
                info = info_data['data']
                if isinstance(info, list):
                    info = next((x for x in info if x.get('id') == usenet_id), info[0] if info else {})
                if not info:
                    xbmc.sleep(sleep_ms)
                    continue
                if info.get('download_finished') or info.get('download_present') or (info.get('download_state') or '').lower() in ('cached', 'completed'):
                    return info.get('files', []) or []
                state = (info.get('download_state') or '').lower()
                if state in ('error', 'dead', 'failed'):
                    xbmc.log(f'[Tinklepad] TB: usenet error state: {state}', xbmc.LOGWARNING)
                    return []
                if i == 0 or i % 10 == 0:
                    xbmc.log(f'[Tinklepad] TB: usenet waiting (state={state}, progress={info.get("progress")})...', xbmc.LOGINFO)
            xbmc.sleep(sleep_ms)
        return []

    def _tb_delete_torrent(self, torrent_id, headers):
        try:
            requests.post(
                f'{TB_BASE}/torrents/controltorrent',
                headers=headers,
                json={'torrent_id': torrent_id, 'operation': 'delete'},
                timeout=10
            )
        except Exception as e:
            xbmc.log(f'[Tinklepad] TB delete error: {e}', xbmc.LOGDEBUG)

    # ==================== UNIFIED RESOLVER ====================
    def resolve(self, url):
        """Try to resolve URL through available debrid services"""
        xbmc.log(f'[Tinklepad] Resolving URL: {url[:50]}...', xbmc.LOGINFO)
        xbmc.log(f'[Tinklepad] RD enabled: {self.rd_enabled()}, authorized: {self.rd_authorized()}', xbmc.LOGINFO)
        xbmc.log(f'[Tinklepad] AD enabled: {self.ad_enabled()}, authorized: {self.ad_authorized()}', xbmc.LOGINFO)
        xbmc.log(f'[Tinklepad] PM enabled: {self.pm_enabled()}, authorized: {self.pm_authorized()}', xbmc.LOGINFO)
        xbmc.log(f'[Tinklepad] TB enabled: {self.tb_enabled()}, authorized: {self.tb_authorized()}', xbmc.LOGINFO)
        
        resolved = None
        
        # Try Real-Debrid first
        if self.rd_authorized():
            xbmc.log('[Tinklepad] Trying Real-Debrid...', xbmc.LOGINFO)
            resolved = self.rd_resolve(url)
            if resolved:
                return resolved, 'RD'
        
        # Try AllDebrid
        if self.ad_authorized():
            xbmc.log('[Tinklepad] Trying AllDebrid...', xbmc.LOGINFO)
            resolved = self.ad_resolve(url)
            if resolved:
                return resolved, 'AD'
        
        # Try Premiumize
        if self.pm_authorized():
            xbmc.log('[Tinklepad] Trying Premiumize...', xbmc.LOGINFO)
            resolved = self.pm_resolve(url)
            if resolved:
                return resolved, 'PM'
        
        # Try Torbox - magnets/torrents AND DDL hoster links (via webdl)
        if self.tb_authorized():
            xbmc.log('[Tinklepad] Trying Torbox...', xbmc.LOGINFO)
            resolved = self.tb_resolve(url)
            if resolved:
                return resolved, 'TB'
        
        xbmc.log('[Tinklepad] No debrid service could resolve the URL', xbmc.LOGWARNING)
        return None, None
    
    def get_status(self):
        """Get status of all debrid services"""
        status = []
        
        if self.rd_authorized():
            username = self.get_token('rd.username') or 'OK'
            status.append(f'[COLOR green]RD: {username}[/COLOR]')
        
        if self.ad_authorized():
            username = self.get_token('ad.username') or 'OK'
            status.append(f'[COLOR green]AD: {username}[/COLOR]')
        
        if self.pm_authorized():
            username = self.get_token('pm.username') or 'OK'
            status.append(f'[COLOR green]PM: {username}[/COLOR]')
        
        if self.tb_authorized():
            username = self.get_token('tb.username') or 'OK'
            status.append(f'[COLOR green]TB: {username}[/COLOR]')
        
        if not status:
            return '[COLOR yellow]No Debrid Authorized - Go to Tools[/COLOR]'
        
        return ' | '.join(status)


# Global instance
debrid_manager = DebridManager()
