...
class DebridManager:
    ...
    def tb_authorize(self):
        ...
        try:
            ...
            if data.get('success') and data.get('data'):
                ...
                # Try email, then customer, then plan name
                username = (
            ...
            xbmcgui.Dialog().ok('Tinklepad', f'Torbox authorization failed:\n{msg}\n\nGet your API key from torbox.app settings')
            return False
        except Exception as e:
    ...
    def tb_resolve(self, url):
        """Resolve a magnet/torrent through Torbox"""
        try:
            token = self.get_token('tb.token')
            if not token:
                return None
            ...
            # Torbox is primarily a torrent debrid service - only handles magnets
            if not url.startswith('magnet:'):
                xbmc.log('[Tinklepad] TB: Non-magnet URL skipped', xbmc.LOGDEBUG)
                return None

            # Step 1: Add magnet to Torbox
            add_response = requests.post(
                f'{TB_BASE}/torrents/createtorrent',
                headers=headers,
                data={'magnet': url, 'seed': 3, 'allow_zip': False},
                timeout=20
            )
            add_data = add_response.json()
            xbmc.log(f'[Tinklepad] TB: Add magnet response: {add_data}', xbmc.LOGDEBUG)

            if not add_data.get('success'):
                xbmc.log(f'[Tinklepad] TB: Failed to add magnet: {add_data.get("detail", "")}', xbmc.LOGWARNING)
                return None

            torrent_id = add_data.get('data', {}).get('torrent_id')
            if not torrent_id:
                return None

            # Step 2: Wait for torrent info / cached check
            ...
            for _ in range(30):
                ...
                if info_data.get('success') and info_data.get('data'):
                    info = info_data['data']
                    # Check if ready (cached or downloaded)
                    if info.get('download_finished') or info.get('cached'):
                        files = info.get('files', []) or []
                        break
                    download_state = info.get('download_state', '')
                    if download_state in ('error', 'dead', 'failed'):
                        xbmc.log(f'[Tinklepad] TB: Torrent error state: {download_state}', xbmc.LOGWARNING)
                        self._tb_delete_torrent(torrent_id, headers)
                        return None
                xbmc.sleep(2000)

            if not files:
                xbmc.log('[Tinklepad] TB: No files ready (not cached)', xbmc.LOGWARNING)
                # Don't delete - let user keep it queued in cloud
                return None

            # Step 3: Pick the largest video file
            video_files = [
                f for f in files
                if str(f.get('short_name', f.get('name', ''))).lower().endswith(video_exts)
            ]
            if not video_files:
                xbmc.log('[Tinklepad] TB: No video files in torrent', xbmc.LOGWARNING)
                return None
            ...
            xbmc.log(f'[Tinklepad] TB: requestdl failed: {dl_data.get("detail", "")}', xbmc.LOGWARNING)
        except Exception as e:
    ...
    def resolve(self, url):
        ...
        # Try Torbox (magnets only)
        if self.tb_authorized() and url.startswith('magnet:'):
