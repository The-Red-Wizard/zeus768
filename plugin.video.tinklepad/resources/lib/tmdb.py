...
# Maps TV network -> placeholder icon file in resources/media/
NETWORK_ICONS = {
    213: 'net_netflix.png', 1024: 'net_prime.png', 2739: 'net_disney.png',
    453: 'net_hulu.png', 2552: 'net_apple.png', 49: 'net_hbo.png', 2697: 'net_hbo.png',
    67: 'net_showtime.png', 16: 'net_cbs.png', 6: 'net_nbc.png', 2: 'net_abc.png',
    19: 'net_fox.png', 174: 'net_amc.png', 4: 'net_bbc1.png', 71: 'net_cw.png',
    88: 'net_fx.png', 614: 'net_paramount.png',
}

# TMDB watch-provider IDs for Movies (US region by default)
MOVIE_PROVIDERS = [
    (8,   'Netflix',            'net_netflix.png'),
    (9,   'Amazon Prime Video', 'net_prime.png'),
    (337, 'Disney+',             'net_disney.png'),
    (15,  'Hulu',                'net_hulu.png'),
    (350, 'Apple TV+',           'net_apple.png'),
    (1899,'HBO Max',             'net_hbo.png'),
    (531, 'Paramount+',          'net_paramount.png'),
    (386, 'Peacock',             'net_peacock.png'),
    (37,  'Showtime',            'net_showtime.png'),
    (526, 'AMC+',                'net_amc.png'),
]
...
def show_networks():
    """Display TV network list (uses per-network placeholder icons when available)"""
    from resources.lib import gui
    
    fallback_icon = os.path.join(gui.MEDIA, 't_nets.png')
    
    for net_id, net_name in sorted(NETWORKS.items(), key=lambda x: x[1]):
        icon_name = NETWORK_ICONS.get(net_id)
        net_icon = os.path.join(gui.MEDIA, icon_name) if icon_name and os.path.exists(os.path.join(gui.MEDIA, icon_name)) else fallback_icon
...
def show_movie_providers():
    """Display streaming-provider list for Movies (uses TMDB watch-providers)."""
    from resources.lib import gui
    
    fallback_icon = os.path.join(gui.MEDIA, 't_nets.png')
    region = ADDON.getSetting('watch_region') or 'US'
    
    for provider_id, name, icon_name in MOVIE_PROVIDERS:
        icon_path = os.path.join(gui.MEDIA, icon_name)
        net_icon = icon_path if os.path.exists(icon_path) else fallback_icon
        li = xbmcgui.ListItem(name)
        li.setArt({'icon': net_icon, 'thumb': net_icon, 'fanart': gui.FANART})
        url = (
            f"{sys.argv[0]}?action=list_content&type=movie&sort=popular"
...
def list_content(handle, params):
    ...
    if params.get('with_keywords'):
        cache_params += f"_k{params['with_keywords']}"
    if params.get('with_watch_providers'):
        cache_params += f"_wp{params['with_watch_providers']}_{params.get('watch_region','US')}"
    ...
    if cached_data:
        ...
        elif sort in ('now_playing', 'upcoming', 'top_rated') and m_type == 'movie':
            url = f'{BASE_URL}/movie/{sort}?api_key={api_key}&page={page}&region=US'
        elif sort in ('airing_today', 'on_the_air', 'top_rated') and m_type == 'tv':
            url = f'{BASE_URL}/tv/{sort}?api_key={api_key}&page={page}'
        else:
            url = f'{BASE_URL}/discover/{m_type}?api_key={api_key}&page={page}&sort_by=popularity.desc'
            # Vote count threshold so anime/documentary discover pages aren't filled with junk
            if params.get('genre_id') in ('16', '99'):
                url += '&vote_count.gte=20'

        # Apply filters (only valid for /discover endpoints)
        if '/discover/' in url:
            if params.get('genre_id'):
            ...
            if params.get('with_keywords'):
                url += f"&with_keywords={params['with_keywords']}"
            if params.get('with_origin_country'):
                url += f"&with_origin_country={params['with_origin_country']}"
            if params.get('with_watch_providers'):
                region = params.get('watch_region', 'US')
                url += (
                    f"&with_watch_providers={params['with_watch_providers']}"
                    f"&watch_region={region}&watch_monetization_types=flatrate"
                )
