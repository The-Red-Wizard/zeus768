...
class TorBox:
    ...
    def resolve_magnet(self, magnet, progress=None):
        ...
        try:
            ...
            last_msg = None
            for i in range(30):
                # Only push a progress update on the first iteration (or when text would change).
                # Calling progress.update() repeatedly with the same text keeps the dialog "alive"
                # and can leave it lingering on top of playback after close().
                if progress and last_msg != 'tb_wait':
                    try:
            ...
            # Switch dialog text away from "Waiting for TorBox..." before the final API call
            # so the user doesn't see that label flash on top of playback.
            if progress:
                try:
                    progress.update(95, 'Starting playback...')
                except Exception:
                    pass
