...
def main_menu():
    ...
    items = [
        ...
        ("[COLOR cyan]PunchPlay[/COLOR] [COLOR red][BETA][/COLOR]", {'action': 'punchplay_menu'}, True, get_icon('trakt'), "PunchPlay.tv tracking & scrobbling (BETA - placeholder, not all endpoints live yet)"),
...
def _close_all_progress_dialogs():
    """Force-close any lingering progress / busy dialogs so they don't sit on top of playback."""
    try:
        xbmc.executebuiltin('Dialog.Close(progressdialog,true)')
        xbmc.executebuiltin('Dialog.Close(progressdialogbg,true)')
...
class _BGProgress:
    """DialogProgress-compatible adapter built on top of DialogProgressBG.

    Why: a modal xbmcgui.DialogProgress can stay visible on top of (or
    behind) the video player when we kick off playback with xbmc.Player()
    directly from the plugin handler - the user reports seeing
    "Resolving with Real-Debrid..." while the stream is already playing.
    DialogProgressBG is non-modal, lives in the bottom-right, and
    disappears cleanly the moment we call .close() - it never collides
    with the player window.

    The adapter mirrors the subset of the DialogProgress API used by
    resources/lib/resolver.py and resources/lib/debrid.py
    (create / update / close / iscanceled) so we don't have to touch
    those files.
    """
    ...
    def create(self, heading, message=''):
        self._heading = heading or 'Orion'
        try:
            self._dlg.create(self._heading, message or '')
        except Exception:
            pass
    ...
    def update(self, percent, message=''):
        if self._closed:
            return
        try:
            self._dlg.update(int(percent), self._heading, str(message or ''))
        except Exception:
            pass
    ...
    def iscanceled(self):
        # Background progress has no Cancel button; user can press Stop
        # on the player instead. Always report not-cancelled.
        return False


def _signal_plugin_done():
    """Tell Kodi the plugin URL finished so it stops showing the 'Working...' notification."""
    try:
...
def play_source(params):
    ...
    FIXED in 3.7.12:
    Previously this method called xbmc.Player().play() directly AND then
    signalled the plugin done with setResolvedUrl(HANDLE, False, ...).
    The False flag tells Kodi "I failed to resolve the URL" - so Kodi
    showed the "Playback failed" dialog AND, because the plugin URL was
    queued as a playlist item, re-invoked the same plugin URL which
    triggered another "Resolving with Real-Debrid..." round-trip.

    The correct Kodi plugin contract is:
      * If invoked from a plugin URL (HANDLE >= 0): resolve once, then
        call setResolvedUrl(HANDLE, True, ListItem(path=stream_url)).
        Kodi takes over playback - no second resolution, no failure
        dialog, no replay loop.
      * If invoked outside a plugin URL context (HANDLE < 0): fall back
        to xbmc.Player().play() directly.
    """
    ...
    has_handle = HANDLE is not None and HANDLE >= 0

    def _fail_handle():
        """Tell Kodi the plugin URL is done (no resolved stream)."""
        if has_handle:
            try:
                xbmcplugin.setResolvedUrl(HANDLE, False, xbmcgui.ListItem())
            except Exception:
    ...
    log(f"play_source called with magnet: {magnet[:60]}... has_handle={has_handle}")

    # Use a non-modal background progress dialog so it can never sit on top
    # of (or behind) the video player once playback starts. See _BGProgress.
    progress = _BGProgress()
    ...
    try:
        ...
        # Reset text first so the very last visible state isn't "Waiting for TorBox..."
        try:
            progress.update(100, ' ')
        except Exception:
            pass
    ...
    try:
        ...
        log(f"add_to_history error: {e}", xbmc.LOGWARNING)

    # Build the playable ListItem.
    # setContentLookup(False) prevents Kodi from issuing a HEAD request that
    # some debrid hosts (especially Real-Debrid hotlinks) treat as a second
    # download attempt and 403 / 404. Setting IsPlayable + the path on the
    # ListItem and handing it back via setResolvedUrl is the canonical Kodi
    # way to play a resolved stream.
    ...
    # --- Hand the resolved stream off to Kodi ---
    if has_handle:
        # Canonical plugin playback: Kodi takes over. NEVER call
        # xbmc.Player().play() here - it races with Kodi's own player
        # and triggers the "Playback failed" / re-resolve loop.
        try:
            xbmcplugin.setResolvedUrl(HANDLE, True, li)
        except Exception as e:
            log(f"setResolvedUrl error: {e}", xbmc.LOGERROR)
            # Last-ditch fallback to direct player
            xbmc.Player().play(stream_url, li)
    else:
        # No plugin handle context (called outside of a plugin URL).
        xbmc.Player().play(stream_url, li)

    # Attach to the active player so we can seek to resume point and
    # monitor progress / autoplay next episode. We don't need to start
    # playback - Kodi already did.
    player = xbmc.Player()
    for _ in range(150):  # up to ~15s for the stream to start buffering
        if player.isPlaying():
            break
        xbmc.sleep(100)
    _close_all_progress_dialogs()
    ...
    if not player.isPlaying():
        log("Player did not start within timeout", xbmc.LOGWARNING)
        return

    # Seek to resume point if any
    if resume_point > 0:
        try:
    ...
    # Setup auto-play next episode + scrobbling if enabled
    if media_type == 'tv' and season and episode:
...
def _monitor_episode_playback(player, tmdb_id, show_title, season, episode, poster, backdrop, database):
    """Monitor episode playback with intro skipping and auto-play next episode"""
    import threading
    ...
    def _monitor():
        try:
            ...
            # Initialize PunchPlay scrobbler if authorized
            pp_scrobbler = None
            if ADDON.getSetting('punchplay_token'):
                try:
                    from resources.lib.punchplay import PunchPlayAPI, PunchPlayScrobbler
                    pp_api = PunchPlayAPI()
                    pp_scrobbler = PunchPlayScrobbler(pp_api)
                    pp_info = {
                        'media_type': 'episode',
                        'title': show_title,
                    ...
                    pp_scrobbler._pp_info = pp_info  # cache for later updates
                    pp_scrobbler.start_watching(pp_info, 0)
                except Exception as e:
                    log(f"PunchPlay scrobble init error: {e}", xbmc.LOGWARNING)
            ...
            # NEW: Calculate when to show Up Next (1 minute before end as requested)
            trigger_time = total_time - 60  # 60 seconds before end
            
            log(f"Up Next monitoring: total={total_time:.0f}s, trigger at {trigger_time:.0f}s")
            
            # NEW: Get intro skip settings
            skip_intro_enabled = ADDON.getSetting('skip_intro_enabled') == 'true'
            skip_intro_auto = ADDON.getSetting('skip_intro_auto') == 'true'
            intro_shown = False
            
            # NEW: Get intro end time from TMDB
            intro_end_time = None
            if skip_intro_enabled:
                try:
                    intro_data = tmdb_api.get_episode_intro_markers(tmdb_id, season, episode)
                    if intro_data:
                        intro_end_time = intro_data.get('intro_end', 90)
                        log(f"Intro skip: will skip to {intro_end_time}s")
                except Exception as e:
                    log(f"Error getting intro markers: {e}", xbmc.LOGWARNING)
                    # Fallback to settings
                    intro_end_time = int(ADDON.getSetting('skip_intro_fallback') or '90')
            ...
            try:
                ...
                # If no next episode in same season, try first episode of next season
                if not next_episode_data:
                    next_season_data = tmdb_api.get_season_episodes(tmdb_id, season + 1)
                    next_season_eps = next_season_data.get('episodes', [])
                    if next_season_eps:
                        ep = next_season_eps[0]
                        still = tmdb_api.get_backdrop_url(ep.get('still_path'))
                        next_episode_data = {
            ...
            while player.isPlaying():
                try:
                    ...
                    # NEW: Handle intro skipping
                    if skip_intro_enabled and intro_end_time and not intro_shown:
                        # Check if we're in the intro timeframe (between 5 seconds and intro_end_time)
                        if 5 < current_time < intro_end_time:
                            intro_shown = True
                            
                            if skip_intro_auto:
                                # Auto-skip intro
                                log(f"Auto-skipping intro to {intro_end_time}s")
                                player.seekTime(intro_end_time)
                                xbmcgui.Dialog().notification('Orion', 'Intro skipped', ADDON_ICON, 2000)
                            else:
                                # Show skip button notification
                                xbmcgui.Dialog().notification(
                                    'Orion', 
                                    f'Intro detected - seek to {int(intro_end_time/60)}:{int(intro_end_time%60):02d} to skip', 
                                    ADDON_ICON, 
                                    5000
                                )
                    ...
                    # Update PunchPlay scrobble progress
                    if pp_scrobbler and progress_pct % 10 == 0:
                        try:
                            info = getattr(pp_scrobbler, '_pp_info', {}) or {}
                            info['duration_seconds'] = int(total_time)
                            pp_scrobbler.progress_watching(progress_pct)
                        except:
                            pass
                    
                    # NEW: Show Up Next dialog (only if next episode exists and feature enabled)
                    auto_next_enabled = ADDON.getSetting('auto_next_episode') == 'true'
                    if auto_next_enabled and next_episode_data and current_time >= trigger_time and not up_next_shown:
                        up_next_shown = True
                        ...
                        if should_play:
                            ...
                            episode_sources({
                                'id': tmdb_id,
                                'title': show_title,
                                'season': next_episode_data['season_number'],
                                'episode': next_episode_data['episode_number']
                            })
                            return
                except:
                    break
            ...
            try:
                ...
                if pp_scrobbler:
                    try:
                        info = getattr(pp_scrobbler, '_pp_info', {}) or {}
                        info['duration_seconds'] = int(total_time)
                        pp_scrobbler.stop_watching(100)
                    except Exception as e:
                        log(f"PunchPlay scrobble stop error: {e}", xbmc.LOGWARNING)
            except:
                pass
...
def punchplay_menu():
    """PunchPlay integration menu - Netflix style or classic (Trakt-parity)"""
    pp_token = ADDON.getSetting('punchplay_token')

    # Always show BETA banner at the top of the menu
    add_directory_item(
        "[B][COLOR red]>>> PunchPlay is in BETA <<<[/COLOR][/B]",
        {'action': 'noop'},
        False,
        plot=("PunchPlay.tv integration is a PLACEHOLDER. The full Trakt-parity "
              "menu is wired up but not every endpoint on the PunchPlay backend "
              "has been implemented yet, so some lists may return empty or fail. "
              "This will be replaced with the live integration once all PunchPlay "
              "API endpoints are public.")
    )
    add_directory_item(
        "[COLOR yellow]Note: not all endpoints are live yet - placeholder until full release[/COLOR]",
        {'action': 'noop'},
        False,
    )
    ...
    if not pp_token:
        add_directory_item(
            "[COLOR red]PunchPlay Not Authorized - Click to Login (BETA)[/COLOR]",
            {'action': 'pair_punchplay'},
            False,
        )
        add_directory_item(
            "[B]About PunchPlay[/B]",
            {'action': 'noop'},
            False,
            plot=("PunchPlay.tv tracks the movies and TV episodes you watch. "
                  "Authorize Orion to scrobble your playback in real-time and "
                  "sync your watchlist, collection, watched history and lists. "
                  "[BETA - placeholder integration, not all endpoints live]")
        )
        xbmcplugin.endOfDirectory(HANDLE)
        return

    # Netflix-style submenu when enabled
    netflix_skin = ADDON.getSetting('netflix_skin_enabled') == 'true'
    use_netflix_submenu = netflix_skin and ADDON.getSetting('use_netflix_submenu') == 'true'
    if use_netflix_submenu:
        _show_punchplay_netflix_style()
        return

    items = [
        ("[B][COLOR gold]My Watchlist - Movies[/COLOR][/B]",
         {'action': 'punchplay_watchlist', 'type': 'movies', 'page': 1}),
        ("[B][COLOR gold]My Watchlist - TV Shows[/COLOR][/B]",
         {'action': 'punchplay_watchlist', 'type': 'shows', 'page': 1}),

        ("[B][COLOR cyan]My Custom Lists[/COLOR][/B]",
         {'action': 'punchplay_my_lists'}),
        ("[B][COLOR magenta]My Liked Lists[/COLOR][/B]",
         {'action': 'punchplay_liked_lists', 'page': 1}),

        ("[B][COLOR lime]My Collection - Movies[/COLOR][/B]",
         {'action': 'punchplay_collection', 'type': 'movies'}),
        ("[B][COLOR lime]My Collection - TV Shows[/COLOR][/B]",
         {'action': 'punchplay_collection', 'type': 'shows'}),

        ("[B]Watched Movies[/B]",
         {'action': 'punchplay_watched', 'type': 'movies'}),
        ("[B]Watched TV Shows[/B]",
         {'action': 'punchplay_watched', 'type': 'shows'}),

        ("[B][COLOR orange]Recommended Movies[/COLOR][/B]",
         {'action': 'punchplay_recommendations', 'type': 'movies', 'page': 1}),
        ("[B][COLOR orange]Recommended TV Shows[/COLOR][/B]",
         {'action': 'punchplay_recommendations', 'type': 'shows', 'page': 1}),

        ("[B]Trending Movies[/B]",
         {'action': 'punchplay_list', 'list': 'trending', 'type': 'movies'}),
        ("[B]Trending TV Shows[/B]",
         {'action': 'punchplay_list', 'list': 'trending', 'type': 'shows'}),
        ("[B]Popular Movies[/B]",
         {'action': 'punchplay_list', 'list': 'popular', 'type': 'movies'}),
        ("[B]Popular TV Shows[/B]",
         {'action': 'punchplay_list', 'list': 'popular', 'type': 'shows'}),
    ]
    for label, params_ in items:
        add_directory_item(label, params_, True)

    add_directory_item("[COLOR yellow]Re-authorize PunchPlay[/COLOR]",
                       {'action': 'pair_punchplay'}, False)
    add_directory_item("[COLOR red]Logout from PunchPlay[/COLOR]",
                       {'action': 'logout_punchplay'}, False)

    xbmcplugin.endOfDirectory(HANDLE)


def _show_punchplay_netflix_style(list_type='watchlist'):
    """Show PunchPlay in Netflix-style submenu (mirrors _show_trakt_netflix_style)"""
    from resources.lib import punchplay, tmdb, submenu

    pp_api = punchplay.PunchPlayAPI()

    row1_items = []
    try:
        movies = pp_api.get_watchlist_movies(1)
        shows = pp_api.get_watchlist_shows(1)
        ...
        for item in movies[:10]:
            ...
            year = movie.get('year', '')
            poster, backdrop, plot, rating, genres = ADDON_ICON, ADDON_FANART, '', 0, ''
            if tmdb_id:
                try:
                    details = tmdb.get_movie_details(tmdb_id)
                    ...
                    pass
            all_items.append({
                'title': title, 'poster': poster, 'backdrop': backdrop,
                'id': tmdb_id, 'media_type': 'movie',
                'rating': rating, 'year': str(year), 'plot': plot, 'genres': genres,
            })
        ...
        log(f"Error loading PunchPlay watchlist: {e}", xbmc.LOGWARNING)

    row2_items = []
    try:
        custom_lists = pp_api.get_user_lists()
        for lst in (custom_lists or [])[:5]:
            name = lst.get('name', 'Unknown')
            list_id = (lst.get('ids') or {}).get('slug', '')
            row2_items.append({'label': f"> {name}", 'action': 'custom_list',
                               'list_id': list_id, 'username': 'me'})
        liked_lists = pp_api.get_liked_lists(1)
        for item in (liked_lists or [])[:5]:
            lst = item.get('list', {})
            user = lst.get('user', {})
            name = lst.get('name', 'Unknown')
            username = user.get('username', 'unknown')
            list_id = (lst.get('ids') or {}).get('slug', '')
            row2_items.append({'label': f"FAV {name}", 'action': 'liked_list',
    ...
    action, selected_item, selected_category = submenu.show_submenu(
        ...
        row2_title='MY LISTS',
        menu_type='trakt'  # reuse trakt skin/look-and-feel
    )
    _handle_punchplay_submenu_result(action, selected_item, selected_category)


def _handle_punchplay_submenu_result(action, selected_item, selected_category):
    """Handle result from Netflix-style PunchPlay submenu"""
    if action == 'back' or action is None:
        return

    if action in ('select_item', 'watch') and selected_item:
        item_id = selected_item.get('id', selected_item.get('tmdb_id'))
        title = selected_item.get('title', '')
        year = selected_item.get('year', '')
        media_type = selected_item.get('media_type', 'movie')
    ...
    if action == 'select_genre' and selected_item:
        ...
        if list_action in ('custom_list', 'liked_list'):
            url = build_url({'action': 'punchplay_list_items',
                             'username': username, 'list_id': list_id})
            xbmc.executebuiltin(f'Container.Update({url})')
        return

    if action == 'category' and selected_category:
        cat = selected_category.get('action', 'watchlist')
        if cat == 'watchlist':
            _show_punchplay_netflix_style('watchlist')
        elif cat == 'collection':
            url = build_url({'action': 'punchplay_collection', 'type': 'movies'})
            xbmc.executebuiltin(f'Container.Update({url})')
        elif cat == 'watched':
            url = build_url({'action': 'punchplay_watched', 'type': 'movies'})
            xbmc.executebuiltin(f'Container.Update({url})')
        elif cat == 'recommendations':
            url = build_url({'action': 'punchplay_recommendations',
                             'type': 'movies', 'page': 1})
            xbmc.executebuiltin(f'Container.Update({url})')
        elif cat == 'trending':
            url = build_url({'action': 'punchplay_list',
                             'list': 'trending', 'type': 'movies'})
            xbmc.executebuiltin(f'Container.Update({url})')
        return
...
def punchplay_watchlist(params):
    """Display PunchPlay watchlist with pagination"""
    from resources.lib import punchplay
    media_type = params.get('type', 'movies')
    page = int(params.get('page', 1))
    pp_api = punchplay.PunchPlayAPI()
    items = (pp_api.get_watchlist_movies(page) if media_type == 'movies'
             else pp_api.get_watchlist_shows(page))
    _display_punchplay_items(items, media_type, 'watchlist')
    if len(items) >= 20:
        add_directory_item(
            f"[B][COLOR cyan]Next Page ({page+1})[/COLOR][/B]",
            {'action': 'punchplay_watchlist', 'type': media_type, 'page': page + 1}
        )
    xbmcplugin.setContent(HANDLE, 'movies' if media_type == 'movies' else 'tvshows')
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_my_lists(params):
    """Display user's custom PunchPlay lists"""
    from resources.lib import punchplay
    pp_api = punchplay.PunchPlayAPI()
    lists = pp_api.get_user_lists() or []
    if not lists:
    ...
    for lst in lists:
        ...
        li = xbmcgui.ListItem(label=display)
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
        url = build_url({'action': 'punchplay_list_items', 'username': 'me',
                         'list_id': list_id, 'list_name': name})
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=True)
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_liked_lists(params):
    """Display user's liked PunchPlay lists"""
    from resources.lib import punchplay
    page = int(params.get('page', 1))
    pp_api = punchplay.PunchPlayAPI()
    liked = pp_api.get_liked_lists(page) or []
    if not liked:
    ...
    for item in liked:
        ...
        li = xbmcgui.ListItem(label=display)
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
        url = build_url({'action': 'punchplay_list_items', 'username': username,
                         'list_id': list_id, 'list_name': name})
        xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=True)
    if len(liked) >= 20:
        add_directory_item(
            f"[B][COLOR cyan]Load More Liked Lists[/COLOR][/B]",
            {'action': 'punchplay_liked_lists', 'page': page + 1}
        )
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_list_items(params):
    """Display items from a specific PunchPlay list"""
    from resources.lib import punchplay, tmdb
    username = params.get('username', 'me')
    list_id = params.get('list_id', '')
    list_name = params.get('list_name', 'List')
    page = int(params.get('page', 1))
    pp_api = punchplay.PunchPlayAPI()
    items = pp_api.get_list_items(username, list_id, page) or []
    if not items:
    ...
    for item in items:
        item_type = item.get('type', 'movie')
        if item_type == 'movie':
            media = item.get('movie', {}); media_type = 'movie'
        elif item_type == 'show':
            media = item.get('show', {}); media_type = 'tv'
        else:
            continue
        title = media.get('title', 'Unknown')
        year = media.get('year', '')
        tmdb_id = (media.get('ids') or {}).get('tmdb')
        poster, backdrop, plot = ADDON_ICON, ADDON_FANART, ''
        if tmdb_id:
            try:
                details = (tmdb.get_movie_details(tmdb_id) if media_type == 'movie'
        ...
        if username == 'me':
            pp_ids = media.get('ids', {}) or {}
            li.addContextMenuItems([(
                'Remove from List',
                f'RunPlugin({build_url({"action": "punchplay_remove_list_item", "list_id": list_id, "list_name": list_name, "type": media_type, "imdb": pp_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})'
            )])
        if media_type == 'movie':
    ...
    if len(items) >= 50:
        add_directory_item(
            ...
            {'action': 'punchplay_list_items', 'username': username,
             'list_id': list_id, 'list_name': list_name, 'page': page + 1}
        )
    xbmcplugin.setContent(HANDLE, 'videos')
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_collection(params):
    from resources.lib import punchplay
    media_type = params.get('type', 'movies')
    pp_api = punchplay.PunchPlayAPI()
    items = (pp_api.get_collection_movies() if media_type == 'movies'
             else pp_api.get_collection_shows())
    _display_punchplay_items(items, media_type, 'collection')
    xbmcplugin.setContent(HANDLE, 'movies' if media_type == 'movies' else 'tvshows')
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_watched(params):
    from resources.lib import punchplay
    media_type = params.get('type', 'movies')
    pp_api = punchplay.PunchPlayAPI()
    items = (pp_api.get_watched_movies() if media_type == 'movies'
             else pp_api.get_watched_shows())
    _display_punchplay_items(items, media_type, 'watched')
    xbmcplugin.setContent(HANDLE, 'movies' if media_type == 'movies' else 'tvshows')
    xbmcplugin.endOfDirectory(HANDLE)


def punchplay_recommendations(params):
    from resources.lib import punchplay, tmdb
    media_type = params.get('type', 'movies')
    page = int(params.get('page', 1))
    pp_api = punchplay.PunchPlayAPI()
    items = (pp_api.get_recommendations_movies(page) if media_type == 'movies'
             else pp_api.get_recommendations_shows(page))
    for item in items:
        title = item.get('title', 'Unknown')
        year = item.get('year', '')
        ...
        if tmdb_id:
            try:
                details = (tmdb.get_movie_details(tmdb_id) if media_type == 'movies'
                           else tmdb.get_tv_details(tmdb_id))
                poster = tmdb.get_poster_url(details.get('poster_path')) or ADDON_ICON
                backdrop = tmdb.get_backdrop_url(details.get('backdrop_path')) or ADDON_FANART
                plot = details.get('overview', '')
    ...
    if len(items) >= 20:
        add_directory_item(
            ...
            {'action': 'punchplay_recommendations', 'type': media_type, 'page': page + 1}
        )
    xbmcplugin.setContent(HANDLE, 'movies' if media_type == 'movies' else 'tvshows')
    xbmcplugin.endOfDirectory(HANDLE)


def _display_punchplay_items(items, media_type, list_type):
    """Helper to display PunchPlay items (mirrors _display_trakt_items)"""
    from resources.lib import tmdb
    for item in items:
        if media_type == 'movies':
            media = item.get('movie', item); mt = 'movie'
        else:
            media = item.get('show', item); mt = 'tv'
        title = media.get('title', 'Unknown')
        year = media.get('year', '')
        tmdb_id = (media.get('ids') or {}).get('tmdb')
        poster, backdrop, plot = ADDON_ICON, ADDON_FANART, ''
        if tmdb_id:
            try:
                details = (tmdb.get_movie_details(tmdb_id) if mt == 'movie'
                           else tmdb.get_tv_details(tmdb_id))
                poster = tmdb.get_poster_url(details.get('poster_path')) or ADDON_ICON
                backdrop = tmdb.get_backdrop_url(details.get('backdrop_path')) or ADDON_FANART
                plot = details.get('overview', '')
        ...
        if list_type == 'watchlist':
            li.addContextMenuItems([(
                ...
                f'RunPlugin({build_url({"action": "punchplay_remove_watchlist", "type": mt, "imdb": pp_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})'
            )])
        elif list_type == 'collection':
            li.addContextMenuItems([(
                'Remove from Collection',
                f'RunPlugin({build_url({"action": "punchplay_remove_collection", "type": mt, "imdb": pp_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})'
            )])
        elif list_type == 'watched':
            li.addContextMenuItems([(
                'Remove from Watched History',
                f'RunPlugin({build_url({"action": "punchplay_remove_history_item", "type": mt, "imdb": pp_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})'
            )])
        if mt == 'movie':
            url = build_url({'action': 'movie_sources', 'id': tmdb_id, 'title': title, 'year': year})
        else:
...
def punchplay_list(params):
    """Display PunchPlay generic list (trending / popular)"""
    from resources.lib import punchplay, tmdb
    list_type = params.get('list', 'trending')
    media_type = params.get('type', 'movies')
    pp_api = punchplay.PunchPlayAPI()
    items = pp_api.get_list(list_type, media_type) or []
    for item in items:
        if media_type == 'movies':
            movie = item.get('movie', item)
            title = movie.get('title', 'Unknown')
...
def trakt_list_items(params):
    ...
    for item in items:
        ...
        # Allow removing items only from the user's own custom lists
        if username == 'me':
            trakt_ids = media.get('ids', {}) or {}
            context_items = [
                ('Remove from List', f'RunPlugin({build_url({"action": "trakt_remove_list_item", "list_id": list_id, "list_name": list_name, "type": media_type, "imdb": trakt_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})')
            ]
            li.addContextMenuItems(context_items)
...
def _display_trakt_items(items, media_type, list_type):
    ...
    for item in items:
        ...
        # Context menu for watchlist / collection / watched
        trakt_ids = media.get('ids', {}) or {}
        if list_type == 'watchlist':
            context_items = [
            ...
            context_items = [
                ('Remove from Collection', f'RunPlugin({build_url({"action": "trakt_remove_collection", "type": mt, "imdb": trakt_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})')
            ]
            li.addContextMenuItems(context_items)
        elif list_type == 'watched':
            context_items = [
                ('Remove from Watched History', f'RunPlugin({build_url({"action": "trakt_remove_history_item", "type": mt, "imdb": trakt_ids.get("imdb", ""), "tmdb": tmdb_id or ""})})')
            ]
            li.addContextMenuItems(context_items)
...
if not action:
    ...
    xbmc.executebuiltin('Container.Refresh')
elif action == 'trakt_remove_collection':
    from resources.lib import trakt
    trakt_api = trakt.TraktAPI()
    ids = {}
    if params.get('imdb'):
    ...
    if params.get('tmdb'):
        ids['tmdb'] = int(params.get('tmdb'))
    trakt_api.remove_from_collection(params.get('type', 'movie'), ids)
    xbmcgui.Dialog().notification('Orion', 'Removed from collection', ADDON_ICON)
    xbmc.executebuiltin('Container.Refresh')
elif action == 'trakt_remove_history_item':
    from resources.lib import trakt
    trakt_api = trakt.TraktAPI()
    ids = {}
    if params.get('imdb'):
    ...
    if params.get('tmdb'):
        ids['tmdb'] = int(params.get('tmdb'))
    trakt_api.remove_from_history(params.get('type', 'movie'), ids)
    xbmcgui.Dialog().notification('Orion', 'Removed from watched history', ADDON_ICON)
    xbmc.executebuiltin('Container.Refresh')
elif action == 'trakt_remove_list_item':
    from resources.lib import trakt
    trakt_api = trakt.TraktAPI()
    ids = {}
    if params.get('imdb'):
    ...
    list_id = params.get('list_id', '')
    item_type = params.get('type', 'movie')
    # The list_items view passes media_type as 'movie' or 'tv' — Trakt API expects 'movie'/'show'
    trakt_type = 'movie' if item_type == 'movie' else 'show'
    if list_id and ids:
        trakt_api.remove_from_list('me', list_id, trakt_type, ids)
        xbmcgui.Dialog().notification('Orion', 'Removed from list', ADDON_ICON)
        xbmc.executebuiltin('Container.Refresh')
    else:
        xbmcgui.Dialog().notification('Orion', 'Could not remove (missing list/id)', ADDON_ICON)
elif action == 'open_settings':
    open_settings()
...
# Service pairing
    ...
    from resources.lib import trakt
    trakt.TraktAPI().pair()
elif action == 'pair_punchplay':
    from resources.lib import punchplay
    punchplay.PunchPlayAPI().pair()
elif action == 'logout_punchplay':
    from resources.lib import punchplay
    punchplay.PunchPlayAPI().logout()
elif action == 'punchplay_menu':
    punchplay_menu()
elif action == 'punchplay_list':
    punchplay_list(params)
elif action == 'punchplay_watchlist':
    punchplay_watchlist(params)
elif action == 'punchplay_my_lists':
    punchplay_my_lists(params)
elif action == 'punchplay_liked_lists':
    punchplay_liked_lists(params)
elif action == 'punchplay_list_items':
    punchplay_list_items(params)
elif action == 'punchplay_collection':
    punchplay_collection(params)
elif action == 'punchplay_watched':
    punchplay_watched(params)
elif action == 'punchplay_recommendations':
    punchplay_recommendations(params)
elif action == 'punchplay_remove_watchlist':
    from resources.lib import punchplay
    pp_api = punchplay.PunchPlayAPI()
    ids = {}
    if params.get('imdb'):
    ...
    if params.get('tmdb'):
        try:
            ...
            pass
    pp_api.remove_from_watchlist(params.get('type', 'movie'), ids)
    xbmcgui.Dialog().notification('Orion', 'Removed from PunchPlay watchlist', ADDON_ICON)
    xbmc.executebuiltin('Container.Refresh')
elif action == 'punchplay_remove_collection':
    from resources.lib import punchplay
    pp_api = punchplay.PunchPlayAPI()
    ids = {}
    if params.get('imdb'):
    ...
    if params.get('tmdb'):
        try:
            ...
            pass
    pp_api.remove_from_collection(params.get('type', 'movie'), ids)
    xbmcgui.Dialog().notification('Orion', 'Removed from PunchPlay collection', ADDON_ICON)
    xbmc.executebuiltin('Container.Refresh')
elif action == 'punchplay_remove_history_item':
    from resources.lib import punchplay
    pp_api = punchplay.PunchPlayAPI()
    ids = {}
    if params.get('imdb'):
    ...
    if params.get('tmdb'):
        try:
            ...
            pass
    pp_api.remove_from_history(params.get('type', 'movie'), ids)
    xbmcgui.Dialog().notification('Orion', 'Removed from PunchPlay watched history', ADDON_ICON)
    xbmc.executebuiltin('Container.Refresh')
elif action == 'punchplay_remove_list_item':
    from resources.lib import punchplay
    pp_api = punchplay.PunchPlayAPI()
    ids = {}
    if params.get('imdb'):
    ...
    list_id = params.get('list_id', '')
    item_type = params.get('type', 'movie')
    pp_type = 'movie' if item_type == 'movie' else 'show'
    if list_id and ids:
        pp_api.remove_from_list('me', list_id, pp_type, ids)
        xbmcgui.Dialog().notification('Orion', 'Removed from PunchPlay list', ADDON_ICON)
        xbmc.executebuiltin('Container.Refresh')
    else:
        xbmcgui.Dialog().notification('Orion', 'Could not remove (missing list/id)', ADDON_ICON)
elif action == 'qr_punchplay':
    from resources.lib import qrcode_helper
    pp_url = ADDON.getSetting('punchplay_url') or 'https://punchplay.tv'
    qrcode_helper.show_qr('PunchPlay', f'{pp_url.rstrip("/")}/link')
# QR Code actions
elif action == 'qr_rd':
    from resources.lib import qrcode_helper
    ...
    from resources.lib import qrcode_helper
    qrcode_helper.show_qr('TorBox', 'https://torbox.app/oauth/device')
