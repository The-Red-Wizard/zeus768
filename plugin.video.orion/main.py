...
def play_source(params):
    ...
    try:
        ...
        if stream_url:
            ...
            # Tell Kodi the plugin URL has finished so the "Working..." spinner disappears
            _signal_plugin_done()

            # Wait briefly for playback to actually start, then nuke any residual progress/busy dialogs
        ...
        try:
            progress.close()  # noqa: F821  - may be undefined if del'd above; ignored on purpose
        except Exception:
            pass
        _close_all_progress_dialogs()
