        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    # Fix: href may already be absolute (http/https or //).
                    # Never blindly concat — that produced broken URLs like
                    # "https://www.torlock2.comhttp://..." which caused hangs
                    # and DNS errors in logs prior to 2.9.8.
                    if href.startswith(('http://', 'https://')):
                        detail_url = href
                    elif href.startswith('//'):
                        detail_url = 'https:' + href
                    else:
                        detail_url = urljoin(self.BASE_URL + '/', href.lstrip('/'))
                    # Only follow links that stay on the TorLock domain.
                    if 'torlock' not in urlparse(detail_url).netloc.lower():
                        continue
                    size = cols[2].get_text(strip=True)
                    seeds = int(re.sub(r'\D', '', cols[3].get_text()) or '0')
