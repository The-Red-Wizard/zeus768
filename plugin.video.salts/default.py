...
def main_menu():
    ...
    items = [
        ...
        {'title': '[B]Cloud[/B] (Google Drive + MEGA)', 'mode': 'cloud_menu'},
...
def cloud_menu():
    """Top-level Cloud menu"""
    from salts_lib import cloud_gdrive, cloud_mega
    items = [
    ...
    if cloud_gdrive.is_authed():
        items.append({'title': '[B]Browse Google Drive[/B]', 'mode': 'cloud_browse',
                      'provider': 'gdrive', 'folder_id': 'root'})
    else:
        items.append({'title': '[COLOR FF8b949e]Google Drive (not connected - see Settings)[/COLOR]',
                      'mode': 'gdrive_auth'})
    if cloud_mega.is_authed():
        items.append({'title': '[B]Browse MEGA.nz[/B]', 'mode': 'cloud_browse',
                      'provider': 'mega', 'folder_id': 'root'})
    else:
        items.append({'title': '[COLOR FF8b949e]MEGA.nz (not connected - see Settings)[/COLOR]',
                      'mode': 'cloud_menu'})
...
def cloud_library_view(kind='all'):
    ...
    for it in items:
        ...
        if it['kind'] == 'tv' and it.get('season') and it.get('episode'):
            ...
            label = title
        provider_tag = '[GD]' if it['provider'] == 'gdrive' else '[MG]'
...
def router(params):
    ...
    # ---- Cloud ----
        ...
        xbmcgui.Dialog().ok(
            'MediaFire Setup',
            'MediaFire closed public developer signup, but SALTS does not '
            'need it. Public folders are walkable anonymously.\n\n'
            '1. Open MediaFire on the web and right-click a folder you '
            'want SALTS to index.\n'
            '2. Choose Share -> "Anyone with the link".\n'
            '3. Copy the folder link (looks like '
            'https://www.mediafire.com/folder/ABC123DEF456/Movies).\n'
            '4. Back here, press "Add MediaFire Public Folder" and paste '
            'the link. Repeat for every folder you want indexed.\n'
            '5. Run "Rescan Cloud Library now" and your videos will show '
            'up under the SALTS cloud library views.')
