...
def movies_menu():
    ...
    items = [
        ...
        {'title': '[COLOR gold]Live Movie Channels[/COLOR]', 'mode': 'movie_channels_menu'},
...
def tvshows_menu():
    ...
    items = [
        ...
        {'title': '[COLOR cyan]New Episodes Today[/COLOR]', 'mode': 'new_episodes_calendar', 'days_back': '0'},
        {'title': '[COLOR lime]Latest Show Premieres[/COLOR]', 'mode': 'latest_premieres', 'page': '1'},
        {'title': '[COLOR gold]📺 Returning Shows (New Seasons)[/COLOR]', 'mode': 'returning_shows'},
        {'title': '[COLOR magenta]📡 Browse by Network[/COLOR]', 'mode': 'network_browser'},
        {'title': '[COLOR orange]⏰ My Episode Countdown[/COLOR]', 'mode': 'episode_countdown'},
...
def new_episodes_calendar(days_back=0):
    ...
    # Add day header
    li = xbmcgui.ListItem(f'[COLOR yellow]━━━ {display_date} ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    ...
    if not data or not data.get('results'):
        li = xbmcgui.ListItem('[COLOR grey]No episodes found for this day[/COLOR]')
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
        xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
        xbmcplugin.endOfDirectory(HANDLE)
        return
...
def latest_premieres(page=1):
    ...
    li = xbmcgui.ListItem(f'[COLOR cyan]━━━ New Premieres (Last 7 Days) - {filter_text} - Page {page}/{total_pages} ({total_results} shows) ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
    ...
    for show in results:
        ...
        label = f'[COLOR lime]NEW[/COLOR] {title} ({year}){country_indicator} - Premiered: {premiere_display}'
    ...
    if page < total_pages:
        li = xbmcgui.ListItem(f'[B][COLOR orange]>> Load More (Page {page + 1})[/COLOR][/B]')
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
...
def returning_shows(page=1):
    ...
    # Add header
    li = xbmcgui.ListItem(f'[COLOR gold]━━━ Returning Shows (New Seasons) - Page {page}/{total_pages} ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
    ...
    for show in results:
        ...
        if next_ep_date:
            try:
                ...
                if delta == 0:
                    days_until = '[COLOR lime]TODAY[/COLOR]'
                elif delta == 1:
                    days_until = '[COLOR cyan]TOMORROW[/COLOR]'
                elif delta > 0:
                    days_until = f'[COLOR yellow]in {delta} days[/COLOR]'
            except:
                pass
...
def network_browser():
    ...
    # Add header
    li = xbmcgui.ListItem('[COLOR magenta]━━━ Browse by Network ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
...
def network_shows(network_id, network_name, page=1):
    ...
    # Add header
    li = xbmcgui.ListItem(f'[COLOR magenta]━━━ {network_name} Shows - Page {page}/{total_pages} ({total_results} total) ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
...
def episode_countdown():
    ...
    if not tracked_shows:
        # Add instruction
        li = xbmcgui.ListItem('[COLOR grey]No shows tracked yet.[/COLOR]')
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
        xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
        
        li = xbmcgui.ListItem('[COLOR cyan]To track a show: Browse to any show, open context menu, select "Track Episode Countdown"[/COLOR]')
        li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
        xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
    ...
    # Add header
    li = xbmcgui.ListItem('[COLOR orange]━━━ My Episode Countdown ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
        ...
        if next_ep:
            ...
            if next_ep_date:
                try:
                    ...
                    if days_until < 0:
                        days_text = '[COLOR grey]AIRED[/COLOR]'
                    elif days_until == 0:
                        days_text = '[COLOR lime]TODAY![/COLOR]'
                    elif days_until == 1:
                        days_text = '[COLOR cyan]TOMORROW[/COLOR]'
                    elif days_until <= 7:
                        days_text = f'[COLOR yellow]{days_until} days[/COLOR]'
                    else:
                        days_text = f'[COLOR white]{days_until} days[/COLOR]'
                except:
                    pass
            ...
            countdown_list.append({
                ...
                'air_date': '',
                'days_until': 999,
                'days_text': '[COLOR grey]No upcoming episodes[/COLOR]'
            })
...
def movie_channels_menu():
    ...
    li = xbmcgui.ListItem('[COLOR orange]⟳ Force Refresh EPG[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    url = build_url({'mode': 'force_refresh_epg'})
    xbmcplugin.addDirectoryItem(HANDLE, url, li, isFolder=False)
    
    # Add Movie Schedules option
    li = xbmcgui.ListItem('[COLOR gold]📅 Movie Schedules (Next 12 Hours)[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    ...
    li = xbmcgui.ListItem(f'[COLOR cyan]All Movie Channels ({len(MOVIE_CHANNELS)})[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
...
def movie_schedules():
    ...
    for hours_ahead in range(0, 12, 2):
        ...
        if hours_ahead == 0:
            slot_display = f'[COLOR lime]NOW[/COLOR] {slot_display}'
        time_slots.append({
...
def movie_schedule_slot(hours_ahead=0):
    ...
    li = xbmcgui.ListItem(f'[COLOR cyan]━━━ {header_text} ━━━[/COLOR]')
    li.setArt({'icon': ADDON_ICON, 'fanart': ADDON_FANART})
    xbmcplugin.addDirectoryItem(HANDLE, '', li, isFolder=False)
    ...
    for channel in MOVIE_CHANNELS:
        ...
        label = f'[B]{ch_name}[/B] [{country}] - [COLOR lime]{movie_title}[/COLOR]'
        if movie_year:
            label += f' ({movie_year})'
...
def movie_channels_category(category):
    ...
    for channel in channels:
        ...
        if current_program:
            label += f' - [COLOR lime]{current_program}[/COLOR]'
