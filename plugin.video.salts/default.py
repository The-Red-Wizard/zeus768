...
def get_sources(title, year='', season='', episode='', media_type='movie', tmdb_id=''):
    ...
    debrid_enabled = (
        ...
        ADDON.getSetting('putio_enabled') == 'true'
    )
    ...
    if magnet:
        ...
        if not stream_url and ADDON.getSetting('torbox_enabled') == 'true':
            ...
            if tb.is_authorized():
                ...
                store_to_cloud = ADDON.getSetting('torbox_store_to_cloud') == 'true'
                stream_url = tb.resolve_magnet(
                    magnet,
                    title=show_title or title,
                    season=season,
                    episode=episode,
                    store_to_cloud=store_to_cloud,
                )
                progress.close()
                if stream_url:
        ...
        # Check Put.io
        if not stream_url and ADDON.getSetting('putio_enabled') == 'true':
            pio = debrid.PutIO()
            if pio.is_authorized():
                progress = xbmcgui.DialogProgress()
                progress.create('SALTS', 'Resolving with Put.io...')
                store_to_cloud = ADDON.getSetting('putio_store_to_cloud') == 'true'
                stream_url = pio.resolve_magnet(
                    magnet,
                    title=show_title or title,
                    season=season,
                    episode=episode,
                ...
                progress.close()
                if stream_url:
                    log_utils.log(f'Resolved via Put.io: {stream_url}', xbmc.LOGINFO)
        
        # No debrid - show message
        if not stream_url:
            xbmcgui.Dialog().ok(ADDON_NAME, 'Torrent sources require a debrid service.\n\nPlease configure Real-Debrid, Premiumize, AllDebrid, TorBox, or Put.io in settings.')
            return
