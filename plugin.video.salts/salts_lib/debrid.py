...
Debrid-Link, Offcloud, LinkSnappy, EasyNews (Usenet)
...
class TorBox:
    ...
    def _request_dl(self, torrent_id, file_id, force_new=False):
        """Request download URL from TorBox.
        
        TorBox URLs are time-limited and may be single-use, so we:
        - Always request a fresh URL (no caching)
        - Add retry logic with exponential backoff
        - Return None if TorBox API fails
        
        Args:
            torrent_id: TorBox torrent ID
            file_id: File ID within the torrent
            force_new: Force a new URL request (for retries)
        """
        max_retries = 3
            ...
            try:
                ...
                if isinstance(dl, dict) and dl.get('success'):
                    ...
                    if url and isinstance(url, str):
                        log_utils.log(
                            f'TorBox: Got fresh URL for torrent={torrent_id} file={file_id} (attempt {attempt + 1})',
                            xbmc.LOGINFO
                        )
                        return url
                    else:
                        log_utils.log_error(f'TorBox requestdl: No URL in response (attempt {attempt + 1})')
                else:
                    error_msg = dl.get('detail') if isinstance(dl, dict) else 'Unknown error'
                    log_utils.log_error(f'TorBox requestdl failed: {error_msg} (attempt {attempt + 1})')
                ...
                log_utils.log_error(f'TorBox requestdl exception: {e} (attempt {attempt + 1})')
                if attempt < max_retries - 1:
                    time.sleep(retry_delay * (attempt + 1))
    ...
    def resolve_magnet(self, magnet, title='', season='', episode='',
        ...
        try:
            ...
            log_utils.log(
                f'TorBox: Successfully got playback URL for torrent={torrent_id}, '
                f'file={chosen.get("id")}. Stream is ready for playback.',
                xbmc.LOGINFO
            )
...
# ==================== EasyNews (Usenet Provider) ====================

class EasyNews:
    """EasyNews Usenet provider with built-in search and streaming"""
    
    BASE_URL = 'https://members.easynews.com'
    SEARCH_URL = 'https://members.easynews.com/2.0/search/solr-search'
    
    def __init__(self):
        self.username = ADDON.getSetting('easynews_username') or ''
        self.password = ADDON.getSetting('easynews_password') or ''
        self.enabled = ADDON.getSetting('easynews_enabled') == 'true'
    
    def is_authorized(self):
        return self.enabled and self.username and self.password
    ...
    def authorize(self, username, password):
        """Set EasyNews credentials"""
        if not username or not password:
            return False
        
        try:
            # Test credentials with a simple search
            test_headers = {
            ...
            code, result = _get(url, headers=test_headers)
            
            if code == 200:
                self.username = username
                self.password = password
                ADDON.setSetting('easynews_username', username)
                ADDON.setSetting('easynews_password', password)
                ADDON.setSetting('easynews_enabled', 'true')
            ...
            log_utils.log_error(f'EasyNews auth error: {e}')
            xbmcgui.Dialog().notification(
                'EasyNews',
                f'Auth error: {e}',
                xbmcgui.NOTIFICATION_ERROR,
    ...
    def revoke(self):
        """Clear EasyNews credentials"""
        self.username = ''
        self.password = ''
        ADDON.setSetting('easynews_username', '')
        ADDON.setSetting('easynews_password', '')
        ADDON.setSetting('easynews_enabled', 'false')
    
    def search(self, query, media_type='video', limit=100):
        """
        Search EasyNews for content
        media_type: 'video', 'audio', 'document', etc.
        """
        if not self.is_authorized():
            return []
        
        try:
            # EasyNews search parameters
            params = {
                'gps': query,
                'sbj': query,
                'st': 'adv',
            ...
            if media_type == 'video':
                params['fex'] = 'm4v,mkv,avi,mp4,mov,wmv,flv,webm'
                params['fty[]'] = 'VIDEO'
            elif media_type == 'audio':
                params['fex'] = 'mp3,flac,aac,ogg,wav'
                params['fty[]'] = 'AUDIO'
            
            url = f'{self.SEARCH_URL}?{urlencode(params)}'
            code, result = _get(url, headers=self._auth_headers())
            ...
            for item in data:
                # Extract file info
                result_item = {
                    'title': item.get('10', ''),  # filename
                    'size': item.get('rawSize', 0),
                    'size_str': self._format_size(item.get('rawSize', 0)),
                    'download_url': item.get('0', ''),  # download URL
                    'stream_url': None,
                    'age_days': 0,
                    'virus': item.get('virus', '0'),
                    'password_protected': item.get('passwd', '0') == '1',
                    'type': 'easynews'
                }
                
                # Generate streaming URL (EasyNews direct HTTP streaming)
                if result_item['download_url']:
                    result_item['stream_url'] = result_item['download_url']
                
                # Calculate age
                try:
                    posted = item.get('posted', 0)
                    if posted:
                        age_seconds = time.time() - float(posted)
                        result_item['age_days'] = int(age_seconds / 86400)
                except:
                    pass
                
                if result_item['title'] and result_item['stream_url']:
                    results.append(result_item)
            
            return results
        
        except Exception as e:
            log_utils.log_error(f'EasyNews search error: {e}')
            return []
    ...
    def search_movie(self, title, year=None):
        ...
        if year:
            query += f' {year}'
        return self.search(query, media_type='video')
    
    def search_tv(self, title, season=None, episode=None):
    ...
    def get_stream_url(self, download_url):
        """Get direct stream URL (EasyNews URLs are already direct)"""
        if not self.is_authorized():
            return None
        return download_url
    ...
    def get_account_info(self):
        ...
        try:
            # EasyNews doesn't have a dedicated account API
            # We can only verify credentials work
            return {
                'username': self.username,
                'status': 'Active',
            ...
            log_utils.log_error(f'EasyNews account info error: {e}')
            return None
    ...
    def resolve_magnet(self, *args, **kwargs):
        """EasyNews doesn't support torrents"""
        return None
    
    def check_cache(self, info_hash):
        """EasyNews doesn't have torrent cache"""
        return False
