...
class TorBox:
    ...
    # ---------- Auth ----------
    def authorize(self):
        """Prompt the user for a TorBox API key and verify it."""
        ...
        # Verify the key against /user/me
        try:
            _, info = _get(
            ...
            log_utils.log_error(f'TorBox auth verify error: {e}')
            xbmcgui.Dialog().notification(
                'TorBox', f'Error: {e}', xbmcgui.NOTIFICATION_ERROR
            )
            return False
    ...
    def resolve_magnet(self, magnet, title='', season='', episode='',
        ...
        try:
            ...
            files = []
            # Wait briefly for cached/ready state and a populated file list
            for _ in range(30):
                ...
                if info and info.get('success'):
                    ...
                    is_ready = (
                        ...
                        or state in ('completed', 'cached', 'uploading',
                                     'downloading', 'metaDL', 'seeding')
                    )
