    ...
    if ADDON.getSetting('sync_enabled') == 'true':
        try:
            ...
            if cloud_sync.is_authed():
                ...
                files = cloud_sync.list_all_videos(
                    progress_cb=lambda s, f:
                        pd.update(min(99, 97 + s),
                                  f'Sync.com: {f} videos found'))
                total += _enrich_and_save('sync', files, pd, base_pct=99)
        except Exception as e:
            xbmc.log(f'cloud_library: sync scan error: {e}',
                     xbmc.LOGWARNING)

    if ADDON.getSetting('filepax_enabled') == 'true':
        try:
            from salts_lib import cloud_filepax
            if cloud_filepax.is_authed():
                pd.update(99, 'Filepax: listing videos...')
                files = cloud_filepax.list_all_videos(
                    progress_cb=lambda s, f: pd.update(
                        min(100, 99 + s // 100),
                        f'Filepax: {f} videos found'))
                total += _enrich_and_save('filepax', files, pd, base_pct=100)
        except Exception as e:
            xbmc.log(f'cloud_library: filepax scan error: {e}',
                     xbmc.LOGWARNING)

    # Degoo
    if ADDON.getSetting('degoo_enabled') == 'true':
        try:
            from salts_lib import cloud_degoo
            if cloud_degoo.is_authed():
                mode = ADDON.getSetting('degoo_mode') or 'account'
                mode_label = 'Account' if mode == 'account' else 'Share Links'
                pd.update(100, f'Degoo ({mode_label}): scanning...')
                files = cloud_degoo.list_all_videos(
                    progress_cb=lambda s, f: pd.update(
                        min(100, 100 + s // 200),
                        f'Degoo ({mode_label}): {f} videos found'))
                total += _enrich_and_save('degoo', files, pd, base_pct=100)
        except Exception as e:
            xbmc.log(f'cloud_library: degoo scan error: {e}',
                     xbmc.LOGWARNING)
...
def get_stream_url(provider, file_id):
    ...
    if provider == 'sync':
        from salts_lib import cloud_sync
        return cloud_sync.get_stream_url(file_id)
    if provider == 'filepax':
        from salts_lib import cloud_filepax
        return cloud_filepax.get_stream_url(file_id)
    if provider == 'degoo':
        from salts_lib import cloud_degoo
        return cloud_degoo.get_stream_url(file_id)
    return None
...
def get_share_url(provider, file_id):
    ...
    if provider == 'sync':
        from salts_lib import cloud_sync
        return cloud_sync.get_share_url(file_id)
    if provider == 'filepax':
        from salts_lib import cloud_filepax
        return cloud_filepax.get_share_url(file_id)
    if provider == 'degoo':
        from salts_lib import cloud_degoo
        return cloud_degoo.get_share_url(file_id)
    return None
