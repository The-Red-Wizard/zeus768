...
class StremioBaseScraper(BaseScraper):
    ...
    def __init__(self, timeout=15):
        ...
        self._addon = xbmcaddon.Addon('plugin.video.salts')
...
class CometScraper(StremioBaseScraper):
    ...
    v1.0.1 (2026-02): Switched primary endpoint to https://comet.feels.legal
    (public Comet instance, no-config /stream path works out of the box).
    Old elfhosted gateways kept as fallback for resilience.
    """
...
# ======================================================================
# METEOR - High-performance streaming addon with debrid support
# (P2P + debrid; supports movie/series/anime/hentai; idPrefixes tt + kitsu)
# ======================================================================

class MeteorScraper(StremioBaseScraper):
    """Meteor - High-performance Stremio addon with debrid support.

    Public no-config endpoint. Returns torrent + P2P streams across
    movie / series / anime / hentai. ~150-300 streams per popular query.

    Added 2026-02 by Zeus768.
    """
    BASE_URL = 'https://meteorfortheweebs.midnightignite.me'
    URL_SETTING = 'meteor_url'
    NAME = 'Meteor'
    is_free = False

    def is_enabled(self):
        addon = xbmcaddon.Addon('plugin.video.salts')
        # Default ON - opt-out via Settings.
        return addon.getSetting('meteor_enabled') != 'false'
